// <auto-generated/>
using System.Collections.Generic;
using System;

namespace CascadeIDE.Services;

internal static class IdeCommandsArgsGenerated
{
    private static readonly Dictionary<string, IdeCommandsArgs.Arg[]> ArgsByCommandId = new(StringComparer.Ordinal)
    {
        ["activate_document"] = new IdeCommandsArgs.Arg[] { new("file_path", "string", true, false, null) },
        ["add_control"] = new IdeCommandsArgs.Arg[] { new("parent_name", "string", true, false, null), new("control_type", "string", true, false, null), new("content", "string", false, false, null), new("name", "string", false, false, null) },
        ["append_agent_notes"] = new IdeCommandsArgs.Arg[] { new("content", "string", true, false, null) },
        ["append_knowledge_file"] = new IdeCommandsArgs.Arg[] { new("file_path", "string", true, false, null), new("content", "string", true, false, null), new("knowledge_path", "string", false, false, null), new("knowledge_root_id", "string", false, false, null), new("save_revision", "boolean", false, false, null) },
        ["apply_edit"] = new IdeCommandsArgs.Arg[] { new("file_path", "string", true, false, null), new("start_line", "integer", true, false, null), new("start_column", "integer", true, false, null), new("end_line", "integer", true, false, null), new("end_column", "integer", true, false, null), new("new_text", "string", true, false, null) },
        ["capture_window"] = new IdeCommandsArgs.Arg[] { new("scope", "string", false, false, null), new("workspace_path", "string", false, false, null), new("output_path", "string", false, false, null) },
        ["casa_field_infer"] = new IdeCommandsArgs.Arg[] { new("query", "string", false, false, null) },
        ["casa_field_query"] = new IdeCommandsArgs.Arg[] { new("query", "string", true, false, null), new("open", "boolean", false, false, null) },
        ["chat_edit_message"] = new IdeCommandsArgs.Arg[] { new("message_id", "string", true, false, null), new("new_content", "string", true, false, null), new("reason", "string", false, false, null) },
        ["chat_export_readable"] = new IdeCommandsArgs.Arg[] { new("write_file", "boolean", false, false, null), new("file_name", "string", false, false, null) },
        ["chat_record_sedm_decision"] = new IdeCommandsArgs.Arg[] { new("outcome", "string", true, false, null), new("chosen_approach", "string", false, false, null), new("selection_rationale", "string", false, false, null), new("considered", "object", false, true, "object"), new("findings", "object", false, true, "object"), new("touched_paths", "string", false, true, "string"), new("revision", "string", false, false, null) },
        ["chat_record_sedm_intent"] = new IdeCommandsArgs.Arg[] { new("outcome", "string", true, false, null), new("trigger", "string", false, false, null), new("chosen_approach", "string", false, false, null), new("selection_rationale", "string", false, false, null), new("considered", "object", false, true, "object") },
        ["chat_select_message"] = new IdeCommandsArgs.Arg[] { new("ordinal", "integer", false, false, null), new("end_ordinal", "integer", false, false, null), new("index", "integer", false, false, null) },
        ["chat_set_product_spine"] = new IdeCommandsArgs.Arg[] { new("line_title", "string", false, false, null), new("current_focus", "string", false, false, null), new("milestones", "string", false, false, null), new("include_in_agent_context", "boolean", false, false, null) },
        ["click_control"] = new IdeCommandsArgs.Arg[] { new("name", "string", false, false, null) },
        ["close_document"] = new IdeCommandsArgs.Arg[] { new("file_path", "string", true, false, null) },
        ["cockpit.open_command_line"] = new IdeCommandsArgs.Arg[] { new("initial_text", "string", false, false, null) },
        ["codebase_index_explain"] = new IdeCommandsArgs.Arg[] { new("workspace_path", "string", false, false, null), new("solution_path", "string", false, false, null), new("hit_id", "integer", true, false, null) },
        ["codebase_index_reindex"] = new IdeCommandsArgs.Arg[] { new("workspace_path", "string", false, false, null), new("solution_path", "string", false, false, null), new("full_rebuild", "boolean", false, false, null) },
        ["codebase_index_search"] = new IdeCommandsArgs.Arg[] { new("workspace_path", "string", false, false, null), new("solution_path", "string", false, false, null), new("query", "string", true, false, null), new("top_n", "integer", false, false, null), new("semantic", "boolean", false, false, null), new("alpha", "number", false, false, null), new("beta", "number", false, false, null), new("vec_top_k", "integer", false, false, null), new("path_prefix", "string", false, false, null), new("exclude_path_prefixes", "string", false, true, "string"), new("extensions", "string", false, true, "string") },
        ["codebase_index_status"] = new IdeCommandsArgs.Arg[] { new("workspace_path", "string", false, false, null), new("solution_path", "string", false, false, null) },
        ["compact_hot_context"] = new IdeCommandsArgs.Arg[] { new("apply", "boolean", false, false, null) },
        ["create_project_in_solution"] = new IdeCommandsArgs.Arg[] { new("template", "string", true, false, null), new("project_name", "string", true, false, null) },
        ["debug_attach"] = new IdeCommandsArgs.Arg[] { new("workspace_path", "string", true, false, null), new("process_id", "integer", true, false, null), new("target_path", "string", false, false, null), new("netcoredbg_path", "string", false, false, null) },
        ["debug_launch"] = new IdeCommandsArgs.Arg[] { new("workspace_path", "string", false, false, null), new("target_path", "string", false, false, null), new("profile_name", "string", false, false, null), new("netcoredbg_path", "string", false, false, null), new("program_args", "string", false, true, "string") },
        ["debug_stop_context"] = new IdeCommandsArgs.Arg[] { new("frame_index", "integer", false, false, null), new("fast", "boolean", false, false, null), new("max_depth", "integer", false, false, null), new("max_children_per_node", "integer", false, false, null), new("time_budget_ms", "integer", false, false, null), new("format", "string", false, false, null), new("json_indented", "boolean", false, false, null) },
        ["debug_variables"] = new IdeCommandsArgs.Arg[] { new("frame_index", "integer", false, false, null) },
        ["delete_knowledge_file"] = new IdeCommandsArgs.Arg[] { new("file_path", "string", true, false, null), new("knowledge_path", "string", false, false, null), new("knowledge_root_id", "string", false, false, null) },
        ["delete_knowledge_section"] = new IdeCommandsArgs.Arg[] { new("file_path", "string", true, false, null), new("section_id", "string", true, false, null), new("knowledge_path", "string", false, false, null), new("knowledge_root_id", "string", false, false, null) },
        ["editor.reveal_code"] = new IdeCommandsArgs.Arg[] { new("code_ref", "string", true, false, null), new("active_file", "string", false, false, null), new("duration_ms", "integer", false, false, null) },
        ["editor.select_code"] = new IdeCommandsArgs.Arg[] { new("code_ref", "string", true, false, null), new("active_file", "string", false, false, null), new("duration_ms", "integer", false, false, null) },
        ["explain_trace_step"] = new IdeCommandsArgs.Arg[] { new("step_index", "integer", true, false, null) },
        ["extract_from_archive"] = new IdeCommandsArgs.Arg[] { new("query", "string", true, false, null), new("revision_file", "string", false, false, null), new("head_limit", "integer", false, false, null), new("context_lines", "integer", false, false, null) },
        ["fetch_web_public_url"] = new IdeCommandsArgs.Arg[] { new("url", "string", true, false, null), new("max_chars", "integer", false, false, null) },
        ["forge.artifact.goto"] = new IdeCommandsArgs.Arg[] { new("bracket", "string", true, false, null), new("base_url", "string", false, false, null), new("select_code", "boolean", false, false, null) },
        ["forge_lens.auth_status"] = new IdeCommandsArgs.Arg[] { new("base_url", "string", false, false, null) },
        ["forge_lens.connect"] = new IdeCommandsArgs.Arg[] { new("base_url", "string", false, false, null) },
        ["forge_lens.create_issue"] = new IdeCommandsArgs.Arg[] { new("title", "string", true, false, null), new("body", "string", false, false, null), new("repo", "string", false, false, null), new("base_url", "string", false, false, null), new("file_path", "string", false, false, null), new("line_start", "integer", false, false, null), new("line_end", "integer", false, false, null), new("member_key", "string", false, false, null) },
        ["forge_lens.create_merge_request"] = new IdeCommandsArgs.Arg[] { new("title", "string", true, false, null), new("source_branch", "string", true, false, null), new("target_branch", "string", false, false, null), new("repo", "string", false, false, null), new("base_url", "string", false, false, null), new("file_path", "string", false, false, null), new("line_start", "integer", false, false, null), new("line_end", "integer", false, false, null) },
        ["forge_lens.disconnect"] = new IdeCommandsArgs.Arg[] { new("base_url", "string", false, false, null) },
        ["fork_chat_thread"] = new IdeCommandsArgs.Arg[] { new("parent_message_id", "string", false, false, null), new("display_title", "string", false, false, null), new("title", "string", false, false, null) },
        ["get_code_metrics"] = new IdeCommandsArgs.Arg[] { new("scope", "string", false, false, null), new("path", "string", false, false, null) },
        ["get_code_navigation_context"] = new IdeCommandsArgs.Arg[] { new("mode", "string", true, false, null), new("file_path", "string", false, false, null), new("line", "integer", false, false, null), new("column", "integer", false, false, null), new("max_related", "integer", false, false, null), new("max_nodes", "integer", false, false, null), new("max_edges", "integer", false, false, null), new("preset", "string", false, false, null), new("include_kinds", "string", false, true, "string"), new("exclude_kinds", "string", false, true, "string"), new("level", "string", false, false, null) },
        ["get_control_appearance"] = new IdeCommandsArgs.Arg[] { new("name", "string", false, false, null) },
        ["get_correspondence_context"] = new IdeCommandsArgs.Arg[] { new("file_path", "string", false, false, null) },
        ["get_editor_content_range"] = new IdeCommandsArgs.Arg[] { new("start_line", "integer", true, false, null), new("end_line", "integer", true, false, null) },
        ["get_editor_state"] = new IdeCommandsArgs.Arg[] { new("max_preview_chars", "integer", false, false, null) },
        ["git_branch"] = new IdeCommandsArgs.Arg[] { new("action", "string", false, false, null), new("name", "string", false, false, null), new("start_point", "string", false, false, null), new("force", "boolean", false, false, null) },
        ["git_commit"] = new IdeCommandsArgs.Arg[] { new("message", "string", true, false, null), new("paths", "string", false, true, "string") },
        ["git_diff"] = new IdeCommandsArgs.Arg[] { new("path", "string", false, false, null), new("staged", "boolean", false, false, null) },
        ["git_fetch"] = new IdeCommandsArgs.Arg[] { new("remote", "string", false, false, null), new("all", "boolean", false, false, null), new("prune", "boolean", false, false, null), new("dry_run", "boolean", false, false, null) },
        ["git_log"] = new IdeCommandsArgs.Arg[] { new("n", "integer", false, false, null) },
        ["git_preflight"] = new IdeCommandsArgs.Arg[] { new("staged", "boolean", false, false, null), new("include_untracked", "boolean", false, false, null), new("include_patches", "boolean", false, false, null) },
        ["git_preflight_fix_safe"] = new IdeCommandsArgs.Arg[] { new("include_patches", "boolean", false, false, null) },
        ["git_pull"] = new IdeCommandsArgs.Arg[] { new("remote", "string", false, false, null), new("branch", "string", false, false, null), new("ff_only", "boolean", false, false, null), new("dry_run", "boolean", false, false, null) },
        ["git_push"] = new IdeCommandsArgs.Arg[] { new("remote", "string", false, false, null), new("branch", "string", false, false, null), new("dry_run", "boolean", false, false, null) },
        ["git_show"] = new IdeCommandsArgs.Arg[] { new("rev", "string", true, false, null), new("path", "string", false, false, null), new("stat_only", "boolean", false, false, null) },
        ["git_submodule"] = new IdeCommandsArgs.Arg[] { new("action", "string", false, false, null), new("path", "string", false, false, null), new("recursive", "boolean", false, false, null) },
        ["go_to_position"] = new IdeCommandsArgs.Arg[] { new("file_path", "string", true, false, null), new("line", "integer", true, false, null), new("column", "integer", true, false, null), new("end_line", "integer", false, false, null), new("end_column", "integer", false, false, null) },
        ["highlight_control"] = new IdeCommandsArgs.Arg[] { new("name", "string", false, false, null) },
        ["ide_agent_sandbox_prepare"] = new IdeCommandsArgs.Arg[] { new("profile", "string", true, false, null), new("workspace_root", "string", true, false, null) },
        ["ide_agent_verify"] = new IdeCommandsArgs.Arg[] { new("policy", "string", true, false, null), new("sandbox_profile", "string", true, false, null), new("solution_path", "string", true, false, null) },
        ["install_ollama_model"] = new IdeCommandsArgs.Arg[] { new("model", "string", true, false, null) },
        ["intercom.agent_provision"] = new IdeCommandsArgs.Arg[] { new("display_name", "string", true, false, null) },
        ["intercom.message_relate"] = new IdeCommandsArgs.Arg[] { new("start_ordinal", "integer", true, false, null), new("end_ordinal", "integer", false, false, null), new("use_selection", "boolean", false, false, null), new("code_ref", "string", false, false, null), new("anchor_json", "object", false, false, null), new("file", "string", false, false, null), new("line_start", "integer", false, false, null), new("line_end", "integer", false, false, null) },
        ["intercom.messages_for_code"] = new IdeCommandsArgs.Arg[] { new("use_selection", "boolean", false, false, null), new("code_ref", "string", false, false, null), new("anchor_json", "object", false, false, null), new("file", "string", false, false, null), new("line_start", "integer", false, false, null), new("line_end", "integer", false, false, null) },
        ["intercom.reveal_attachment"] = new IdeCommandsArgs.Arg[] { new("anchor_json", "object", false, false, null), new("file", "string", false, false, null), new("line_start", "integer", false, false, null), new("line_end", "integer", false, false, null), new("member_key", "string", false, false, null), new("syntax_scope", "object", false, false, null), new("duration_ms", "integer", false, false, null), new("select", "boolean", false, false, null) },
        ["intercom.server_start"] = new IdeCommandsArgs.Arg[] { new("base_url", "string", false, false, null) },
        ["list_agent_notes_revisions"] = new IdeCommandsArgs.Arg[] { new("limit", "integer", false, false, null) },
        ["list_knowledge_files"] = new IdeCommandsArgs.Arg[] { new("subdir", "string", false, false, null), new("knowledge_path", "string", false, false, null), new("knowledge_root_id", "string", false, false, null) },
        ["load_solution"] = new IdeCommandsArgs.Arg[] { new("path", "string", true, false, null) },
        ["memory_health"] = new IdeCommandsArgs.Arg[] { new("active_scope", "string", false, false, null) },
        ["move_document_to_group_1"] = new IdeCommandsArgs.Arg[] { new("file_path", "string", true, false, null) },
        ["move_document_to_group_2"] = new IdeCommandsArgs.Arg[] { new("file_path", "string", true, false, null) },
        ["move_document_to_group_3"] = new IdeCommandsArgs.Arg[] { new("file_path", "string", true, false, null) },
        ["open_chat_clarification_batch"] = new IdeCommandsArgs.Arg[] { new("batch_json", "string", true, false, null) },
        ["open_docs_template"] = new IdeCommandsArgs.Arg[] { new("path", "string", false, false, null) },
        ["open_file"] = new IdeCommandsArgs.Arg[] { new("path", "string", true, false, null) },
        ["read_hot_context"] = new IdeCommandsArgs.Arg[] { new("active_scope", "string", false, false, null) },
        ["read_knowledge_file"] = new IdeCommandsArgs.Arg[] { new("file_path", "string", true, false, null), new("knowledge_path", "string", false, false, null), new("knowledge_root_id", "string", false, false, null), new("offset", "integer", false, false, null), new("limit", "integer", false, false, null) },
        ["read_workspace_file"] = new IdeCommandsArgs.Arg[] { new("file_path", "string", true, false, null), new("offset", "integer", false, false, null), new("limit", "integer", false, false, null), new("max_chars", "integer", false, false, null) },
        ["remove_breakpoint"] = new IdeCommandsArgs.Arg[] { new("file_path", "string", true, false, null), new("line", "integer", true, false, null) },
        ["request_confirmation"] = new IdeCommandsArgs.Arg[] { new("message", "string", true, false, null) },
        ["reveal_editor_range"] = new IdeCommandsArgs.Arg[] { new("file_path", "string", true, false, null), new("start_line", "integer", false, false, null), new("end_line", "integer", false, false, null), new("member_key", "string", false, false, null), new("syntax_scope", "object", false, false, null), new("duration_ms", "integer", false, false, null) },
        ["rollback_agent_notes"] = new IdeCommandsArgs.Arg[] { new("revision_file", "string", false, false, null) },
        ["rollback_trace_step"] = new IdeCommandsArgs.Arg[] { new("step_index", "integer", true, false, null) },
        ["route_context"] = new IdeCommandsArgs.Arg[] { new("query", "string", true, false, null), new("active_scope", "string", false, false, null), new("max_sections", "integer", false, false, null), new("max_chars", "integer", false, false, null) },
        ["run_affected_tests"] = new IdeCommandsArgs.Arg[] { new("changed_paths", "string", false, true, "string") },
        ["run_code_cleanup"] = new IdeCommandsArgs.Arg[] { new("include_path", "string", false, false, null) },
        ["save_document"] = new IdeCommandsArgs.Arg[] { new("file_path", "string", false, false, null), new("content", "string", false, false, null) },
        ["search_agent_notes"] = new IdeCommandsArgs.Arg[] { new("query", "string", true, false, null), new("head_limit", "integer", false, false, null) },
        ["search_web_public_query"] = new IdeCommandsArgs.Arg[] { new("query", "string", true, false, null) },
        ["search_workspace_text"] = new IdeCommandsArgs.Arg[] { new("pattern", "string", true, false, null), new("subpath", "string", false, false, null), new("fixed_string", "boolean", false, false, null), new("glob", "string", false, false, null), new("max_matches", "integer", false, false, null), new("rg_path", "string", false, false, null) },
        ["select"] = new IdeCommandsArgs.Arg[] { new("file_path", "string", true, false, null), new("start_line", "integer", true, false, null), new("start_column", "integer", true, false, null), new("end_line", "integer", true, false, null), new("end_column", "integer", true, false, null) },
        ["send_chat"] = new IdeCommandsArgs.Arg[] { new("message", "string", false, false, null), new("role", "string", false, false, null) },
        ["send_keys"] = new IdeCommandsArgs.Arg[] { new("keys", "string", true, false, null), new("name", "string", false, false, null) },
        ["set_breakpoint"] = new IdeCommandsArgs.Arg[] { new("file_path", "string", true, false, null), new("line", "integer", true, false, null), new("condition", "string", false, false, null) },
        ["set_build_output_visible"] = new IdeCommandsArgs.Arg[] { new("visible", "boolean", true, false, null) },
        ["set_code_navigation_map_level"] = new IdeCommandsArgs.Arg[] { new("level", "string", true, false, null) },
        ["set_control_layout"] = new IdeCommandsArgs.Arg[] { new("name", "string", true, false, null), new("layout", "string", true, false, null) },
        ["set_control_text"] = new IdeCommandsArgs.Arg[] { new("name", "string", true, false, null), new("text", "string", true, false, null) },
        ["set_focus"] = new IdeCommandsArgs.Arg[] { new("name", "string", false, false, null) },
        ["set_git_panel_visible"] = new IdeCommandsArgs.Arg[] { new("visible", "boolean", true, false, null) },
        ["set_instrumentation_dock_visible"] = new IdeCommandsArgs.Arg[] { new("visible", "boolean", true, false, null) },
        ["set_mfd_region_expanded"] = new IdeCommandsArgs.Arg[] { new("visible", "boolean", true, false, null) },
        ["set_mfd_shell_page"] = new IdeCommandsArgs.Arg[] { new("page", "string", true, false, null) },
        ["set_panel_size"] = new IdeCommandsArgs.Arg[] { new("panel", "string", true, false, null), new("width", "integer", false, false, null), new("height", "integer", false, false, null) },
        ["set_pfd_region_expanded"] = new IdeCommandsArgs.Arg[] { new("visible", "boolean", true, false, null) },
        ["set_primary_work_surface"] = new IdeCommandsArgs.Arg[] { new("surface", "string", true, false, null) },
        ["set_secondary_shell_page"] = new IdeCommandsArgs.Arg[] { new("page", "string", true, false, null) },
        ["set_terminal_visible"] = new IdeCommandsArgs.Arg[] { new("visible", "boolean", true, false, null) },
        ["set_ui_language"] = new IdeCommandsArgs.Arg[] { new("culture", "string", true, false, null) },
        ["set_ui_mode"] = new IdeCommandsArgs.Arg[] { new("mode", "string", true, false, null) },
        ["set_ui_theme"] = new IdeCommandsArgs.Arg[] { new("theme", "string", true, false, null) },
        ["show_preview"] = new IdeCommandsArgs.Arg[] { new("title", "string", true, false, null), new("content", "string", true, false, null) },
        ["submit_chat_clarification_response"] = new IdeCommandsArgs.Arg[] { new("response_json", "string", true, false, null) },
        ["toggle_pin_document"] = new IdeCommandsArgs.Arg[] { new("file_path", "string", true, false, null) },
        ["upsert_agent_notes_section"] = new IdeCommandsArgs.Arg[] { new("section_id", "string", true, false, null), new("content", "string", true, false, null) },
        ["upsert_knowledge_section"] = new IdeCommandsArgs.Arg[] { new("file_path", "string", true, false, null), new("section_id", "string", true, false, null), new("content", "string", true, false, null), new("knowledge_path", "string", false, false, null), new("knowledge_root_id", "string", false, false, null), new("save_revision", "boolean", false, false, null) },
        ["write_agent_notes"] = new IdeCommandsArgs.Arg[] { new("content", "string", true, false, null) },
        ["write_knowledge_file"] = new IdeCommandsArgs.Arg[] { new("file_path", "string", true, false, null), new("content", "string", true, false, null), new("knowledge_path", "string", false, false, null), new("knowledge_root_id", "string", false, false, null), new("save_revision", "boolean", false, false, null) },
    };

    public static bool TryGetArgs(string commandId, out IdeCommandsArgs.Arg[] args) =>
        ArgsByCommandId.TryGetValue(commandId, out args);

}
