...
public static class IdeCommands
{
    ...
    /// <summary>Прочитать только hot-context (L0/L1) без архивного хвоста. args: active_scope?:string; returns: json; example: {"active_scope":"current-projects"}.</summary>
    ...
    /// <summary>Health-check памяти: размер hot-context и рекомендации. args: active_scope?:string; returns: json; example: {"active_scope":"current-projects"}.</summary>
    ...
    /// <summary>Записать knowledge-файл в канон (полная замена). args: file_path:string, content:string, canon_path?:string, save_revision?:boolean; returns: text; example: {"file_path":"META/x.md","content":"# Hi","save_revision":true}.</summary>
    public const string WriteKnowledgeFile = "write_knowledge_file";
    /// <summary>Добавить блок в конец knowledge-файла. args: file_path:string, content:string, canon_path?:string, save_revision?:boolean; returns: text; example: {"file_path":"META/x.md","content":"more","save_revision":true}.</summary>
    public const string AppendKnowledgeFile = "append_knowledge_file";
    /// <summary>Вставить/обновить секцию в knowledge-файле по section_id. args: file_path:string, section_id:string, content:string, canon_path?:string, save_revision?:boolean; returns: text; example: {"file_path":"index.md","section_id":"foo","content":"body"}.</summary>
    public const string UpsertKnowledgeSection = "upsert_knowledge_section";
    /// <summary>Удалить knowledge-файл. args: file_path:string, canon_path?:string; returns: text; example: {"file_path":"tmp.md"}.</summary>
    public const string DeleteKnowledgeFile = "delete_knowledge_file";
    /// <summary>Удалить секцию из knowledge-файла. args: file_path:string, section_id:string, canon_path?:string; returns: text; example: {"file_path":"index.md","section_id":"foo"}.</summary>
    ...
    /// <summary>Запустить отладку (netcoredbg DAP): workspace_path, target_path (.dll/.exe) — цель задаёшь явно; опционально netcoredbg_path, program_args. Пример target — тестовая samples/DebugTarget, чтобы не дебажить саму IDE. returns: text; example: {"workspace_path":"D:\\\\proj","target_path":"samples\\\\DebugTarget\\\\bin\\\\Debug\\\\net10.0\\\\DebugTarget.dll"}.</summary>
