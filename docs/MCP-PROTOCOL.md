...
| `intercom.reveal_attachment` | Reveal из ленты по AttachmentAnchor: open + re-resolve + transient highlight (ADR 0128 §8, 0130 фаза 1). args: anchor_json?:object, file?:string, line_start?:integer, line_end?:integer, member_key?:string, select?:boolean; returns: text; example: {"file":"src/Foo.cs","line_start":10,"line_end":25}. |
...
| `reveal_editor_range` | Показать диапазон строк в редакторе transient-подсветкой без изменения selection (ADR 0130). args: file_path:string, start_line:integer, end_line:integer; returns: text; example: {"file_path":"C:\\tmp\\a.cs","start_line":10,"end_line":25}. |
...
Секция **`workspace_layout.border_brush`** задаёт рамки трёх колонок (PFD / Forward / MFD), вертикальные сплиттеры и шов между зонами (`CascadeTheme.WorkspacePanelBorderBrush`). Если секции нет, используется `editor_column.border_brush`.
