...
| `MainWindowViewModel.ShellConstruction.cs` | 263 | Конструктор и композиция shell: дочерние VM, шина, DAP/HCI, топология presentation (ADR 0017). |
...
| `MainWindowViewModel.WorkspaceNavigationMap.Bridge.cs` | 53 | Мост к `WorkspaceNavigationMapViewModel` (ADR 0039): host + lifecycle, без прокси-привязок. |
