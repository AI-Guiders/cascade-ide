...
|------|-----------------|-------------------|-------------------|
| **Любопытство** | **Папка** workspace (project-aware корень), один разговор с агентом: *«как ты вообще думаешь?»*, одна гипотеза, один `project-id` когда прояснится | Практика → вопросы → playbook/ADR в каноне → MCP и продукт **по мере боли**, не «сначала всё выучить» | Любая среда с доступом к репо и агенту; маркеры `[PRIMARY:…]`, `[SCOPE:…]` — по мере надобности (см. KB `playbook-multi-project-context-v1`) |
| **Интегрированный** | Уже собранный контур: router, hot-context, product cockpit, ADR, три входа команд | Быстрее для тех, кто **внутри**; опасен как **единственная** демонстрация скептику | [SHOWCASE](https://github.com/AI-Guiders/kb-public) → индекс KB; в CIDE — [handbook](design/cide-design-handbook-v1.md), Intercom, ADR |
...
**Project-aware:** «папка» — не абстракция, а **конкретный** `workspace_path` и при необходимости **primary** / **scope** в треде, чтобы агент не смешивал репозитории и не тащил весь канон в контекст. IOP не требует сначала выучить карту MCP — требует **не путать** продукты, когда их несколько.
