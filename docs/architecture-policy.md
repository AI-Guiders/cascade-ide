...
| PFD **instrument deck** — варианты состава и поверхностей (SA, метрики кода, карта кода, ADR indicator); таблица-каталог, критерии отбора; не смешивать с [0063](adr/0063-instrument-deck-named-composition-one-anchor.md) как нормативом терминов | [0073](adr/0073-pfd-instrument-deck.md) (Proposed) |
...
| Инструмент кабины: дескриптор слота композитора (`CockpitInstrumentDescriptor`), не Avalonia-контрол; SE vs карта кода как разные `instrument_id` в PFD | [0047](adr/0047-cockpit-instrument-descriptor-and-slot-composition.md) (Accepted · Implemented) |
...
| Карта кода на PFD: поток управления в методе (условные ветки, схождение), KISS; Roslyn CFG как источник; расширение subgraph / MCP | [0053](adr/0053-semantic-map-control-flow-pfd.md) (Accepted · Implemented) |
...
| **Категории инструментов** и **типы графов** (`graph_kind`): ось домена + структура подграфа; «Карта кода» = **карта намерений кода** (не «любой граф связей»); опциональные поля дескриптора / JSON — по дорожной карте | [0065](adr/0065-instrument-categories-domain-taxonomy.md) (Accepted) |
...
| **Graph-backed surfaces** — общий контракт для графовых экранов (карта кода, GitMap, будущие); измерения: данные, interaction, навигация, layout, selection, sync с workspace | [0067](adr/0067-graph-backed-surfaces-contract.md) (Accepted) |
...
- **v1.60** — [0039](adr/0039-workspace-navigation-affordances.md): продуктовая метафора (шкаф vs карта боя), граф релевантного контекста, PFD/MFD, карта кода и `presentation`.  
...
- **v1.74** — `Cockpit/Cds/CockpitSurfaceState` (schema `0.3`) получил `instruments` как проекцию HostSurface-кадра для MCP/наблюдаемости; добавлен `Cockpit/Surface/MainWindowInstrumentMountRegistry` (`instrument_id → mount`, хост-слой Avalonia/Skia, без UI карты кода на этом шаге).
...
- **v1.83** — [0053](adr/0053-semantic-map-control-flow-pfd.md): карта кода и поток управления на PFD (условные ветки, KISS, Roslyn CFG, subgraph/MCP); статус **Accepted · Implemented**.
...
- **v2.00** — добавлен [0065](adr/0065-instrument-categories-domain-taxonomy.md): **категории инструментов** как ось домена (CodeNavigation / WorkspaceNavigation / топология repo); ортогонально слоту и `instrument_id`; «Карта кода» — граф, не прибор; опциональное `instrument_category` в дескрипторе — по мере необходимости; статус **Accepted**.
- **v2.01** — [0065](adr/0065-instrument-categories-domain-taxonomy.md): «Карта кода» в узком смысле — **карта намерений кода**; введена ось **`graph_kind`** (тип графа в wire/MCP); таблица минимальных значений.
...
- **v2.08** — добавлен [0073](adr/0073-pfd-instrument-deck.md): PFD instrument deck — каталог вариантов (SA, code metrics, карта кода, ADR indicator…), критерии размещения; **Proposed**, дополняет [0021](adr/0021-pfd-mfd-cockpit-attention-model.md) и [0063](adr/0063-instrument-deck-named-composition-one-anchor.md).
