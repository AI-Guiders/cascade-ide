...
| PFD **instrument deck** — варианты состава и поверхностей (SA, метрики кода, Semantic Map, ADR indicator); таблица-каталог, критерии отбора; не смешивать с [0063](adr/0063-instrument-deck-named-composition-one-anchor.md) как нормативом терминов | [0073](adr/0073-pfd-instrument-deck.md) (Proposed) |
...
| **Intercomm:** пользовательское имя канала связи в IDE (не только LLM); модель «участники + система»; **командный масштаб** — внешний стек/API по умолчанию, не дублировать в IDE; алиасы discoverability; код `chat*` — strangler ([0080](adr/0080-intercomm-naming-and-multi-party-channel-model.md)) | [0080](adr/0080-intercomm-naming-and-multi-party-channel-model.md) (Proposed) |
...
| **Graph-backed surfaces** — общий контракт для графовых экранов (Semantic Map, GitMap, будущие); измерения: данные, interaction, навигация, layout, selection, sync с workspace | [0067](adr/0067-graph-backed-surfaces-contract.md) (Accepted) |
...
- **v2.08** — добавлен [0073](adr/0073-pfd-instrument-deck.md): PFD instrument deck — каталог вариантов (SA, code metrics, Semantic Map, ADR indicator…), критерии размещения; **Proposed**, дополняет [0021](adr/0021-pfd-mfd-cockpit-attention-model.md) и [0063](adr/0063-instrument-deck-named-composition-one-anchor.md).
...
- **v2.15** — добавлен [0080](adr/0080-intercomm-naming-and-multi-party-channel-model.md): **Intercomm** как продуктовое имя канала связи (агент, команда, система); открытые вопросы UI/i18n; **Proposed**; строка в таблице «Где что зафиксировано».
- **v2.16** — [0080](adr/0080-intercomm-naming-and-multi-party-channel-model.md): §5 готовый командный контур (интеграция/API) vs собственная реализация; ссылка на [0035](adr/0035-mfd-embedded-webview-external-llm-and-mcp-boundary.md) для риска WebView; открытый вопрос п.5; индекс README и строка навигатора уточнены.
