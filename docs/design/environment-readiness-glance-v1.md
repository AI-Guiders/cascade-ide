...
**MVP (код):** `Views/EnvironmentReadinessSecondaryPageView.axaml` — оверлей во вторичном контуре (`SecondaryShellView`, v1 — колонка зоны Mfd) при `ShowEnvironmentReadinessPage`; открытие: палитра `show_environment_readiness_page` («Готовность окружения…»), закрытие: `close_environment_readiness_page` или кнопка «Назад к нижней панели». Снимок строк: `Models/EnvironmentReadinessItem`, сборка — `Services/EnvironmentReadinessSnapshotBuilder`. Тесты: `CascadeIDE.Tests/EnvironmentReadinessSnapshotBuilderTests.cs`.
