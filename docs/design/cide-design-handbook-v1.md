...
!!! tip "Маршрут для дизайнера (без ADR)"
    0. **[IOP-манифест](../iop-manifest-v1.md)** — дисциплина; **[§ Cascade use case](../iop-manifest-v1.md#пример-экосистема-cascade)** — экосистема; **§1.1** — дизайн CIDE. KB — SHOWCASE, отдельно.  
...
### 1.1 Два порога входа (CIDE)

IOP — **[манифест](../iop-manifest-v1.md)**; экосистема — **[use case](../iop-manifest-v1.md#пример-экосистема-cascade)**. Ниже — только **дизайн Cascade IDE**:

| В IOP-манифесте | В CIDE (этот handbook) |
|-----------------|-------------------------|
| Папка + разговор | Workspace + Intercom/composer или Cursor в том же репо — **без** обязательного §2 и D1–D9 |
| Интегрированный контур | §2, [intercom-design-hub](intercom-design-hub-v1.md), ADR при споре с инженерами |

**Для дизайна:** первый экран скептику — **одна поверхность + один честный вопрос**, не стена терминов. **Анти-паттерн:** «сначала выучи handbook» при демо коллеге.
...
### 2.0 Восприятие важнее авторинга (design thesis)

Классические IDE заточены под **написание кода**: редактор — центр, а всё сопутствующее — решения, intent, история рассуждения — размазано по панелям и файлам и **читается плохо**. В agentic-цикле **писать код в основном берёт агент**; работа человека — **понимать, направлять, фиксировать решения**. Редактор остаётся для проверки и точечных правок по запросу (reveal), не как дефолтная «станция набора».

**Для дизайна:** полировка и measure cap — на **Intercom / Surface** (читать, ориентироваться, ветвить). Chrome и редактор — тихие, пока оператор не открыл attach. Session graph — не «красивый чат», а **удобное восприятие сессии**. Канон: [0172](../adr/0172-conversation-first-habitat.md), [VDS v1](cide-vds-v1.md), KB `kb-cide-brand-positioning-v1.md`.
...
**Партнёр для проектирования:** агент в том же контуре — для **обсуждения и спора до кода** (снять ветвления → ADR → быстрая реализация), без замены вкуса и ответственности человека — [philosophy §8](cascadeide-philosophy-v1.md#8-агент-как-партнёр-для-проектирования-до-кода), [§8.1 «память, не лычка»](cascadeide-philosophy-v1.md#81-память-команды-не-лычка-в-промпте), [IOP manifest](../iop-manifest-v1.md).
...
| Slash, **attach / якоря** | [0128](../adr/0128-intercom-attachment-anchors-and-code-references.md) · [0130](../adr/0130-editor-agent-range-reveal-without-selection.md) · [0131](../adr/0131-editor-slash-select-code-by-bracket-reference.md) · [0119](../adr/0119-chat-slash-commands-intercom-surface.md), [0124](../adr/0124-slash-parametric-editor-line-commands.md)–[0125](../adr/0125-slash-workspace-file-commands-and-dynamic-completion.md) |
| **Fenced / MD в ленте** | [0129](../adr/0129-intercom-message-body-markdown-and-fenced-code.md) · [0069](../adr/0069-markdown-preview-tool-surface-and-renderer-decoupling.md) |
