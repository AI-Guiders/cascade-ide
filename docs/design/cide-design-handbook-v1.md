...
!!! tip "Маршрут для дизайнера (без ADR)"
    0. **§1.1** — если входишь «с нуля» или ведёшь онбординг: два порога (любопытство vs готовый кокпит).  
...
### 1.1 Два порога входа

Продукт и документация **не обязаны** начинаться с полного кокпита. Два легитимных входа ([philosophy §9](cascadeide-philosophy-v1.md#9-два-пути-входа-любопытство-и-интегрированный-контур)):

| Порог | Старт | Не путать с |
|-------|--------|-------------|
| **Любопытство** | Workspace (папка), разговор с агентом, одна задача — *как ты думаешь?* | «Слабая версия» без IOP |
| **Интегрированный** | Handbook §2, Intercom hub, ADR по спору | Обязательный билет для всех |

**Для дизайна и онбординга:** макеты и copy не должны создавать впечатление, что без §2 и D1–D9 нельзя начать. Первый экран для скептика — **одна поверхность + один честный вопрос**, не стена терминов. Второй путь — для тех, кто уже внутри контура.

**Анти-паттерн:** «сначала выучи философию» при демо коллеге. **Норма:** та же папка и диалог, с которых начинался продукт; дисциплина нарастает, когда без неё больно.
