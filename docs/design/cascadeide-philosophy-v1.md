...
## 9. Два пути входа (любопытство и интегрированный контур)

Система выросла так, что **снаружи** кажется: сначала философия, IOP, отказ от «инструментального мышления», потом можно пользоваться. Это **ложный единственный вход** — он описывает зрелый контур, но не историю его появления.

| Путь | С чего начинают | Что накапливается | Для кого |
|------|-----------------|-------------------|----------|
| **Любопытство** (канонический старт) | Папка workspace, один разговор с агентом: *«как ты вообще думаешь?»*, одна гипотеза | Практика → вопросы → ADR/playbook → MCP и IDE **по мере надобности** | Новичок, скептик, коллега «дай кнопку» |
| **Интегрированный** | Уже готовый кокпит: Intercom, `command_id`, handbook, ADR | Быстрее для тех, кто **внутри**; выше порог, если показать только вершину | Команда на продукте, дизайнер по [handbook](cide-design-handbook-v1.md) |

Оба пути **сходятся**: один контур (агент и человек — те же команды и артефакты), одна дисциплина намерения. Разница — **порядок подачи**, не «упрощённая vs настоящая» версия.

**Ошибка онбординга:** требовать сразу второй путь («прочитай манифест, иначе не поймёшь») — отталкивает тех, кто мог бы зайти с первого. **Ошибка упрощения:** обещать, что первый путь обойдётся без дисциплины — со временем без явного намерения и верификации контур деградирует в «чат с кнопками».

**Практика для приглашающего:** не «сначала вся философия», а **та же точка, с которой начинали вы** — папка, диалог, одна задача; философия и ADR — **наложение**, когда появляется боль от хаоса, а не билет на вход. См. [handbook §1.1](cide-design-handbook-v1.md#11-два-порога-входа).
...
| 2026-05-19 | §9: два пути входа (любопытство vs интегрированный контур); handbook §1.1. |
