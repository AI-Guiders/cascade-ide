...
4. `WorkspaceHealthSegments` обновляется; привязка к `WorkspaceHealthStripView` (через `WorkspaceChromeBandView`) или к `WorkspaceHealthMfdPageView` в колонке MFD при `DedicatedPage`.
