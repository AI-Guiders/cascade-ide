...
| EOL inlay `var` → тип | `CSharpLanguageService.GetVarInlayHintsForFile` + `EditorInlayHintBackgroundRenderer` (последний в Z-order) | 0085 inlay; не intra-line ([AvaloniaEdit #429](https://github.com/AvaloniaUI/AvaloniaEdit/discussions/429)) | sRGB **Editor HUD** = `EditorHudDiagnosticsChroma` + `EditorHud.Severity*` в `App.axaml` (MFD/Problems/волны). |
