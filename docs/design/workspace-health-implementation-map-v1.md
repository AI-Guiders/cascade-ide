...
| **Снимок Workspace Health** | `WorkspaceHealthInputSnapshot`: нормализованные входы (build/tests/debug/git) до компоновки поверхности. Не привязан к форме «полоски». |
| **Композиция поверхности канала** | `WorkspaceHealthSurfaceCompositor`: из снимка собирает упорядоченные `WorkspaceHealthSegment` (порядок, флаги вроде `IsBuildRunning`). Отвечает за **состав канала** Workspace Health, не за пиксели и не за зону PFD/MFD. |
| **Раскладка зоны / страницы (chrome layout)** | Куда на экране попадают блоки: полоса снизу, сетка на странице MFD, карточка в PFD. Задаётся **пресетом** и шаблонами (AXAML) и/или отдельным слоем в коде; рабочее имя в дизайне — *layout страницы зоны* / *display page layout*. Только **геометрия контейнера** в зоне, не дублирует порядок build/tests — тот уже зафиксирован `WorkspaceHealthSurfaceCompositor`. |
| **Поверхность (surface)** | **Слой представления:** как **показать** те же сегменты (полоса, страница, карточка в хроме). Выбор Strip vs Page (`workspace_health_surface`, enum `WorkspaceHealthUiSurface`) — пресет и разметка; **снимок и `WorkspaceHealthSurfaceCompositor` не зависят** от этого слоя. Это не «хост событий» и не шина сообщений — только UI. Каноническое имя оси «форма» в ADR: **`ContentRepresentation`** (`Strip` / `Page`); см. [ADR 0063 § две оси](../adr/0063-instrument-deck-named-composition-one-anchor.md#adr0063-two-axes). |
...
Типы `WorkspaceHealth*` задают **смысл** сегментов (build/tests/debug/git); [`WorkspaceHealthStripView`](../../Views/WorkspaceHealthStripView.axaml) — одна из **поверхностей представления**; при странице MFD / блоке PFD те же данные идут в **другую разметку** без смены `WorkspaceHealthSurfaceCompositor`.

**Показ и данные развязаны:** **Strip** или **Page** — это только **слой представления** (настройки пресета), не ветвление логики снимка и не хост событий. Strip отнимает высоту у лобового; Page не отнимает ту же полосу, но требует перехода взгляда — пользователь сам решает, что важнее, **не меняя** источники данных и `WorkspaceHealthSurfaceCompositor`.
...
Несколько источников (сборка, тесты, отладка, git) **подают состояние и строки**; **один** слой (`WorkspaceHealthSurfaceCompositor`) задаёт **порядок и состав** сегментов — независимо от **слоя представления** (**Strip** или **Page**). Текущая разметка полосы: [`WorkspaceHealthStripView`](../../Views/WorkspaceHealthStripView.axaml) (Balanced/Focus vs Power cockpit).
...
| Композитор поверхности | `Cockpit/Composition/WorkspaceHealth/WorkspaceHealthSurfaceCompositor.cs` | `Compose(ObservableCollection<WorkspaceHealthSegment>, WorkspaceHealthInputSnapshot, WorkspaceHealthSurfaceDecision)`; порядок: Build → Tests → Debug → Git; `IsBuildRunning` только на сегменте Build. |
...
| Канал снимка | `Cockpit/Channels/WorkspaceHealth/IWorkspaceHealthChannel.cs`, `WorkspaceHealthProvider.cs` | `Build(WorkspaceHealthChannelContext)` собирает входы из делегатов и DAP (строки git и счётчики отладки подставляет `MainWindowViewModel` с `Chrome` / `InstrumentationPanel`). Сам канал **не** ссылается на `UiChromeViewModel`. |
| VM | `ViewModels/MainWindowViewModel.WorkspaceHealth.cs` | `RebuildWorkspaceHealth()` вызывает `_workspaceHealth.Build(...)` и `_workspaceHealthSurfaceCompositor.Compose(...)`. |
...
|----------|----------------|
| Принцип ADR: единый слой смысла над источниками (не разрозненные toast) | Да: канал Workspace Health — `_workspaceHealth.Build(...)` + `_workspaceHealthSurfaceCompositor.Compose(...)` + `WorkspaceHealthInputSnapshot`. |
...
4. **Раскладка без нижней полосы** — Page / карточка PFD; тот же снимок + `WorkspaceHealthSurfaceCompositor`, **другой слой представления** (§1, §7.5).
5. **Канал снимка** — сделано: `IWorkspaceHealthChannel`, `WorkspaceHealthProvider`, `WorkspaceHealthFormat`.
...
|--------|------------|---------|------------------|
| **Workspace Health** | Ориентир «что происходит с задачей» (build/tests/debug/git) | Сборка, тесты, сессия отладки, git | **Не применяется** — фиксированный канонический порядок в `WorkspaceHealthSurfaceCompositor` (Build → Tests → Debug → Git) |
...
flowchart LR
  subgraph work [Workspace Health]
    ...
    Comp[WorkspaceHealthSurfaceCompositor]
...
- **`WorkspaceHealthSurfaceCompositor`:** читает не сырой TOML, а **уже слитый** `WorkspaceHealthLayoutPolicy` (immutable) на один вызов `Compose`, чтобы тестировать без файлов.

### 7.5 Page и нижняя полоса

Тот же `WorkspaceHealthInputSnapshot` и тот же порядок сегментов подаются в **другую разметку представления** (Page): полноэкранная страница MFD или блок в PFD. Меняются только шаблоны (не одна строка на сегмент, допускается многострочный вывод). `WorkspaceHealthSurfaceCompositor` **не** ветвится по Strip/Page.

### 7.6 Фазы внедрения (предложение)

| Фаза | Содержание | Критерий готовности |
|------|------------|---------------------|
| **F1** | Политика пустых сегментов + тесты на `WorkspaceHealthSurfaceCompositor` / нормализацию | Предсказуемое поведение в Focus vs Power |
