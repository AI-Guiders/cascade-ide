...
| Компонент | Путь | Роль |
|-----------|------|------|
| Снимок входов | `Cockpit/Channels/IdeHealth/IdeHealthInputSnapshot.cs` | `IdeHealthInputSnapshot` + `IdeHealthSegmentInput` (build/tests/debug/git). Точка расширения без раздувания сигнатур. |
...
5. **Канал снимка** — сделано: `IIdeHealthChannel`, `IdeHealthProvider`, `IdeHealthFormat`.
...
- **EICAS:** сортировка по **уровню** и времени/идентификатору; дубликаты сообщений — политика слияния (одна строка на источник). Связь с **escalation** и таймерами — в [ADR §5](../adr/0021-pfd-mfd-cockpit-attention-model.md), не в `IdeHealthFormat`.
