...
| **Поверхность (surface)** | Способ **показать** те же сегменты: полоса, страница, карточка в хроме и т.д. Один снимок и композитор смысла могут кормить разные поверхности; какая поверхность активна — из пресета и раскладки зоны. |
...
**Цена места у strip:** нижняя полоса постоянно отнимает высоту у лобового — разумный компромисс «статус всегда в поле взгляда», но если приоритет — **площадь редактора**, пресет может: скрыть полосу (`telemetry_strip`), перенести ту же телеметрию на **страницу MFD** или **компактный блок PFD**, опереться на HUD — без смены снимка и композитора смысла, только смена **поверхности** и **раскладки зоны**.
