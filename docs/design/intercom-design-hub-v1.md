...
**Проектирование до макетов:** сложные домены (D4 attach, D6 клик по chip) выгодно **долго проговаривать** с агентом в том же IOP-контуре — снять ветвления, оформить ADR/playbook, потом рисовать PNG и кодировать MVP. Вход «папка + диалог» vs «готовый кокпит» — [IOP-манифест § Два порога](../iop-manifest-v1.md#два-порога-входа-project-aware); агент как спарринг — [philosophy §8](cascadeide-philosophy-v1.md#8-агент-как-партнёр-для-проектирования-до-кода).
