...
- **@mention axes (thin shipped 2026-08-07):** Seat `@PF`/`@PM` · Kind `@guest`/`@citizen`/`@operator` · Who `@Sierra`/`@Света`/`@Kir`. Wake = f(kind): guest→external cannon (Cursor AutoI today); citizen|operator→Glass Face. Seat follows occupant kind (e.g. Sierra·citizen @PF → Face, not Cursor). `GlassIntercomMention.ResolveWakes` + FanOut sinks.
