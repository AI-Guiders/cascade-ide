...
|---------|---------------------|------------------------|
| **MCP → IDE, UI-поток** | Маршалинг `ExecuteCommandAsync`, фазы миграции 0–5 ([architecture-migration.md](../architecture-migration.md)). | Расширять команды по мере сценариев агента; держать контракт [MCP-PROTOCOL.md](../MCP-PROTOCOL.md) в актуальном виде. |
