...
- Slash → **`command_id`** ([0030](0030-command-ids-hotkeys-and-ui-registry-layers.md)); catalog is a **projection** of the registry + curated aliases, not a second executor in the VM.
- **Autocomplete is required** (namespace → action hierarchy, hints, `/help`) — extended catalog is not accepted without it.
...
- **Slash catalog** (`ChatSlashCommandCatalog`) is a **mapping** (alias → `command_id` + arg template), built from:
  1. **Curated** table in code (v1);
  2. v2+ — **projection** of a subset of `IdeCommandPaletteCatalog` / `IdeCommands` metadata (palette title need not match slash; alias is explicit).
...
Without autocomplete, extension to `/build`, `/test`, … is **not accepted** — the operator must not memorize namespaces and actions.
...
| **C** | Extend aliases (`/git`, `/nav`, palette projection) | by discoverability, not big-bang |
