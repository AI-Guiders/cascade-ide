...
## 9. Статус интеграций (кратко, обновляем по мере работы)

Зафиксировано здесь, чтобы не искать по чатам: что уже есть в продукте, что сознательно отложено.

| Тема | Статус |
| ---- | ------ |
| **Cursor ACP** (внешний `cursor-agent` по stdio, ответы в чате IDE) | Работает: провайдер в настройках, путь к `cursor-agent.cmd` / `dist-package`, UTF-8 для перенаправленных потоков (русский текст в ответах). См. SDK ACP в `externals/acp-csharp`. |
| **MCP к IDE** (сервер по `--mcp-stdio`, инструменты, единый контур с агентом и PFD/MFD) | Следующая доработка — после стабилизации сценария чата и приоритетов UX. |
| **Чат** (контекст, раскладка, глубина истории) | Итерации по §5–§8 и отдельным UX-заметкам; не смешивать с закрытием задачи по MCP. |
