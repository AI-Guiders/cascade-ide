...
| Главное окно на весь экран при `(xP+yF)` на первом экране | `MainWindow` `Loaded` | `PresentationRequestsMainWindowMaximized` ← `PresentationLayoutAnalyzer.IsPfdForwardCombinedOnFirstScreen` | ✅ | `WindowState.Maximized`; иначе дефолт размера из XAML. |
