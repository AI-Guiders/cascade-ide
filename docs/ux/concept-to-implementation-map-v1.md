...
| Build/Test/Debug + event timeline | `IdeHealthStripView` (Power cockpit) + вкладка «События» | `EventTimeline`, `IsTerminalVisible` | ✅ | В Power дубль IDE Health на вкладке «Терминал» отключён (`IdeHealthOnTerminalTab`), чтобы не сжимать консоль; лента — «События». |
