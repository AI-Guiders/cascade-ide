...
- **Меню (Menu):**
  ...
  - **Вид:** Обозреватель решения, Вывод сборки, Чат, Терминал, док инструментирования (вкладки «События / Тесты / Отладка» при включённом доке — во всех режимах, в т.ч. Focus); Режим интерфейса (Focus / Balanced / Power — радио); Тема (Светлая, Тёмная, Как Cursor, Открыть файл темы…); Превью в отдельном окне.
...
**Power:** на корне `BottomPanelView` — `Classes.power="{Binding IsPowerMode}"` (в `MainWindow` у контрола `DataContext` привязан к `RootWindow`, чтобы биндинг не терялся). У `Border#BottomPanelShell` в Power — скругление верхних углов и усиленная тень (`views|BottomPanelView.power` в стилях). Телеметрия сборки/тестов/Git — в `TelemetryStripView` под редактором; дубль на вкладке «Терминал» не показывается (`ShowPowerTelemetryOnTerminalTab`), при входе в Power выбирается вкладка «Терминал» (`BottomPanelTabIndex = 0`).
...
*Версия: 1. Соответствует MainWindow.axaml по состоянию на 2026-03. При изменении разметки обновить этот документ.*ет MainWindow.axaml по состоянию на 2026-03. При изменении разметки обновить этот документ.*