...
**Готовность окружения (отдельный канал от Workspace Health):** быстрый ответ «всё ли ок» — LSP, транспорт агента, доступность нужных исполняемых файлов (в т.ч. через PATH на Windows/Linux), и **только те переменные окружения / пути, которые IDE сама использует**; не дамп всего `environ`. Не замена экрана настроек при редактировании. Чертёж: [`environment-readiness-glance-v1.md`](../design/environment-readiness-glance-v1.md).
