...
**Workspace Health** — каноническое имя этого канала в продукте и ADR (опора сильнее, чем у размытого «telemetry»). Реализация в типах `WorkspaceHealth*` (см. чертёж реализации). В русских подписях UI рядом уместно **состояние воркспейса**. Ранние черновики: «телеметрия работы», затем «операционная телеметрия». Сводный лексикон и таблица эволюции имён: [0022](0022-workspace-health-lexicon.md).
...
**В Cascade это другой слой:** не замена оконному менеджеру ОС, а **настройка пресета** раскладки IDE (пропорции колонок, видимость хрома, Workspace Health (strip vs page) и т.д. — см. [0010](0010-ui-modes-toml-configuration.md), чертёж [`workspace-health-compositor-implementation-v1.md`](../design/workspace-health-compositor-implementation-v1.md)). Инвариант ADR сохраняется: **не** перетаскивать семантику PFD/MFD/EICAS между зонами в обычной рабочей сессии — только смена пресета / конфигурации (§16).
