...
**Статус:** Proposed  
**Дата:** 2026-04-06  
**Обновлено:** 2026-04-11 — каноническое имя контура: **Workspace Health** (сборка, тесты, отладка, git); в русских текстах рядом допустимо **состояние воркспейса**. Ранее: «операционная телеметрия» / «телеметрия работы». Ранее: §1.2: слот презентации vs канал; `DedicatedPage` / `telemetry_surface` (только Workspace Health). Якорь: `#glossary-presentation-vs-channel`. Ранее: 2026-04-06 — §«Плагины и модель внимания»: обязательная привязка расширений к зоне/каналу/пресету. Ранее: §19: ссылка на живой чертёж [`onboarding-first-run-v1.md`](../design/onboarding-first-run-v1.md). Ранее: §ARINC 661: расшифровка **CDS** (Cockpit Display System). Ранее: §«Архитектура зон»: содержимое якорей PFD/MFD vs термин Page у канала Workspace Health (`telemetry_surface`). Ранее под §«Архитектура зон»: **якоря vs attention flow** (не смешивать). Ранее: §1.1 **ProcessHost** (MCP/ACP) vs UI; уточнение «канал ≠ `*Host*` View». Ранее: §1.1 словарь «канал / слой представления / имена в коде»; §5: EICAS как канал (не синоним полосы / не третья колонка); уточнение «Расположение». Ранее: §17 подпункт «Редактор раскладки (FancyZones-аналог)»; переносимые идеи ARINC 661; §5 Escalation; голос; §2.1; §19; §16/§17; три зоны, HUD, §18 DX acceptance.  
...
| **Слой представления** | Как и где в **разметке** показать те же данные: полоса (Strip), страница зоны (Page), оверлей, карточка — по пресету (`telemetry_surface`, AXAML). Не шина событий, не маршрутизация сообщений, не «хост событий». |
