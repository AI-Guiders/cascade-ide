...
**В Cascade это другой слой:** не замена оконному менеджеру ОС, а **настройка пресета** раскладки IDE (пропорции колонок, видимость хрома, Workspace Health (strip vs page) и т.д. — см. [0010](0010-ui-modes-toml-configuration.md), чертёж [`workspace-telemetry-compositor-implementation-v1.md`](../design/workspace-telemetry-compositor-implementation-v1.md)). Инвариант ADR сохраняется: **не** перетаскивать семантику PFD/MFD/EICAS между зонами в обычной рабочей сессии — только смена пресета / конфигурации (§16).
