...
**Соответствие бытовым словам и лампам deck:** в продукте часто говорят «Error / Warning / Information» (как у диагностик IDE). В каноне EICAS здесь **три** яруса W/C/A; отдельная буква «I» не вводится — **Information** = **Advisory** (A). **Error** / критичная недоступность сервиса ≈ **Warning** (W, красный). **Warning** в смысле «обрати внимание, не катастрофа» ≈ **Caution** (C, янтарь). У ламп annunciator в коде `AnnunciatorLampLevel` имена исторические: `Unavailable` → W, `Warning` → C, `Info` → A; цвета — `CockpitPrimitivesPalette` (ADR 0064).
