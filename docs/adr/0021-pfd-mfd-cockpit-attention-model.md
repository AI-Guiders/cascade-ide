...
| **Границы ответственности** | Зоны `frontal` / `pfd` / `mfd` / `eicas` и пресеты ([§2](#2-соответствие-зонам-cascade), [§2.1](#21-слои-конфигурации-продукт-пользователь-workspace-репозиторий)): что может оказаться в PFD vs только в MFD — **политика**, а не гонка плагинов за z-order. |
