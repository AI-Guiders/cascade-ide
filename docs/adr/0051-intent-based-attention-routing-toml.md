...
- не пересекается по смыслу с `instrument_placement_rules` (ADR 0050).
...
- Нужна ли отдельная пользовательская секция в `%LocalAppData%\\CascadeIDE\\settings.toml` для routing’а (аналогично метрикам), или достаточно bundle+repo.
- Нужно ли разделять `terminal` vs `terminal_dock` как intent (в зависимости от будущей архитектуры dock’а).
