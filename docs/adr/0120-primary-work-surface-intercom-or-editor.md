...
**Design thesis (ось, не только layout):** классические IDE оптимизируют **авторинг**; сопутствующая информация вокруг кода читается плохо. В agentic-цикле **писать в основном агент**, человеку — **восприятие и управление сессией**; ручное редактирование вторично (verify, surgical fix). `primary_work_surface = intercom` — продуктовое выражение этой оси; habitat и session graph — [0172](0172-conversation-first-habitat.md).
