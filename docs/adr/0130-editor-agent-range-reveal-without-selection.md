...
| [0131](0131-editor-slash-select-code-by-bracket-reference.md) | `/editor select code [M:…]` — тот же bracket-parse, **select** в буфере, не attach |
...
| **3** *(done)* | `[intercom.attachments.code].navigate` в TOML; MCP `intercom.reveal_attachment` без `select` → дефолт из настроек; `IntercomAttachmentNavigator` для UI-клика (Shift → select) |
...
- `[intercom.attachments.code]` `navigate = "reveal"` (ожидаемый дефолт) → scroll + transient highlight;
- `navigate = "select"` → `SelectInEditor` после open/re-resolve;
- `reveal_load_solution = "when_needed"` \| `"never"` — грузить .sln перед reveal (см. `IntercomAttachmentsCodeSettings`);
...
| 2026-05-19 | Accepted: фазы 0–3 в коде (`IntercomSettings`, `IntercomAttachmentNavigator`, MCP default `select`). |
| 2026-05-20 | **Accepted · Implemented**; кластер с [0128](0128-intercom-attachment-anchors-and-code-references.md) / [0131](0131-editor-slash-select-code-by-bracket-reference.md). |
