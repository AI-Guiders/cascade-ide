...
| **3** *(done)* | `[intercom].attachment_navigate` в TOML; MCP `intercom.reveal_attachment` без `select` → дефолт из настроек; `IntercomAttachmentNavigator` для UI-клика (Shift → select) |
