...
**Статус:** Proposed  
**Дата:** 2026-04-14  
**Обновлено:** 2026-04-14 — разбор `mcp.json` ↔ CIDE; workspace «открыть папку» для сценариев по репо; **реализация «Открыть папку» в UI и MCP** (см… Подробности — [§ История](#adr0048-history).  
## Связанные ADR
...
- Дублирование процессов (IDE + отдельный MCP-процесс IDE) допустимо; оптимизация «один процесс» — отдельное архитектурное решение: [0082](0082-acp-ide-mcp-loopback-single-process.md) (loopback HTTP/SSE MCP в GUI вместо второго `CascadeIDE --mcp-stdio` для ACP).
...
| Дата | Изменение |
|------|-----------|
| 2026-04-14 | разбор `mcp.json` ↔ CIDE; workspace «открыть папку» для сценариев по репо; **реализация «Открыть папку» в UI и MCP** (см. [§ workspace](#adr0048-folder-workspace)). |
