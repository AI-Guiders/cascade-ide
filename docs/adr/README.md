...
- **Статусы (в шапке ADR и в колонке «Статус»):** жизненный цикл решения — **Accepted** | **Superseded** | **Deprecated**. При необходимости через **« · »** добавляют второй смысл (ортогонально решению): **Implemented** (есть согласованная реализация в коде), **частично**, **отложено**, **в работе** и т.п. Каталоги по статусам не используем — один плоский `docs/adr/`, чтобы не ломать ссылки.
