...
**Дата:** 2026-04-02  
**Обновлено:** 2026-04-11 — якоря `adr0010-p1`…`p8` в разделе «Решение» и ссылки на [п. 3](#adr0010-p3), [п. 7](#adr0010-p7); соглашение — [README ADR](README.md#adr-anchors-policy). 2026-04-11 — поле **`presentation`** или **`zone_screen_layout`** в `workspace.toml` (строка раскладки) согласовано с [0017](0017-multi-window-workspace-and-agent-surfaces.md) [п. 4](0017-multi-window-workspace-and-agent-surfaces.md#adr0017-p4); токены разбора — [0017 грамматика](0017-multi-window-workspace-and-agent-surfaces.md#adr0017-presentation-grammar). 2026-04-08 — намерение задать **топологию презентации** зон в TOML после появления альтернатив одному `MainGrid` ([0017](0017-multi-window-workspace-and-agent-surfaces.md)); см. подраздел ниже.  
...
**Имя поля** в `workspace.toml` / overlay (snake_case): канонично **`presentation`** — строка в **нотации скобок** из [0017](0017-multi-window-workspace-and-agent-surfaces.md) («Несколько мониторов»), например `(PFD+forward) (Mfd)`; **внутри одной пары скобок** **`+`** и **`|`** — взаимозаменяемые стили записи якорей на одном дисплее (**`(PFD|forward|Mfd)`** = **`(PFD+forward+Mfd)`**). Формальные роли символов (`screen_markers`, `screen_separator`, `zone_separator`) — в [0017 § грамматика](0017-multi-window-workspace-and-agent-surfaces.md#adr0017-presentation-grammar). **Альтернативное имя ключа** — **`zone_screen_layout`** (то же значение и смысл, если нужна большая однозначность); в одном конфиге задаётся **одно** из двух. Перечисление значений `AttentionLayoutSurfaceKind` и **валидация** строки (в т.ч. предупреждение при топологии из `workspace.toml` репозитория) — в реализации; отдельный ADR под схему TOML **не требуется**, пока объём правил укладывается в этот подпункт.
