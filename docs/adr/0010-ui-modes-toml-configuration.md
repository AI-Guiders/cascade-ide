...
**Имя поля** в `workspace.toml` / overlay (snake_case): канонично **`presentation`** — строка раскладки из [0017](0017-multi-window-workspace-and-agent-surfaces.md) («Несколько мониторов»), например `(PFD+forward) (Mfd)` или кратко **`(P+F) (M)`** ([алиасы якорей](0017-multi-window-workspace-and-agent-surfaces.md#adr0017-anchor-aliases)); **внутри одной пары скобок** при дефолтной грамматике **`+`** и **`|`** — взаимозаменяемые стили якорей на одном дисплее (**`(PFD|forward|Mfd)`** = **`(PFD+forward+Mfd)`**). **Опционально** рядом — ключи **`screen_markers`**, **`screen_separator`**, **`zone_separator`** (настраиваемая грамматика; **EBNF** и дефолты в [0017](0017-multi-window-workspace-and-agent-surfaces.md#adr0017-presentation-ebnf)). **Альтернативное имя ключа** — **`zone_screen_layout`** (то же значение и смысл, если нужна большая однозначность); в одном конфиге задаётся **одно** из двух. Перечисление значений `AttentionLayoutSurfaceKind` и **валидация** строки (в т.ч. предупреждение при топологии из `workspace.toml` репозитория) — в реализации; отдельный ADR под схему TOML **не требуется**, пока объём правил укладывается в этот подпункт.
