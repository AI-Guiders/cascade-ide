...
При добавлении в `UiModeLayoutRegistry` (или аналога) режим Debug задаётся отдельной `UiModeLayoutSpec`: ожидаемо **дерево включено**, **терминал и вывод сборки включены**, **`ChatPanelExpanded: false`** (чат в Cursor — не в колонке IDE), **число групп редактора** — 1–2. Точные значения фиксируются в коммите, который вводит режим.
