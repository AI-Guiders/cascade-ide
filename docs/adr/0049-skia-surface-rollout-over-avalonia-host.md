...
8. **Базовая slot-policy для production-пресетов (`instrument_id -> slot_id`):**
...
10. **`instrument_id -> instrument_class` вводится как incremental registry v1 (source of truth):**
   ...
   - цель: единая классификация (`heavy` / `sa` / `hybrid`) для slot-policy и guardrail-проверок без расползания по устным договоренностям.
...
14. **Дизайн рассматривается как измеряемая SA-гипотеза (L1/L2/L3), а не только как инженерный rollout:**
   - каждая `slot_policy` обязана иметь целевые сценарии и SA-профиль по уровням:
