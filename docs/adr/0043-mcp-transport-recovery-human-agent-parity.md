...
На практике MCP-серверы — это **процессы**, поднятые **хостом** (например Cursor): обмен по stdio с дочерним процессом CascadeIDE с `--mcp-stdio`, плюс другие серверы (Roslyn, отладка и т.д.) в той же экосистеме настроек.
