...
3. **Уровни детализации (Glance / Normal / Inspect) — прежде всего Declutter, не только геометрия.** Доменный уровень детализации (в т.ч. общие `glance` / `normal` / `inspect` из плана внедрения) задаёт **отбор и агрегацию до Layout**. Layout меняет метрики раскладки, но не подменяет решение «сколько сущностей показывать» между Glance и Inspect. *(Пример: `SemanticMapDetailLevel` для карты; для другого инструмента — свой enum/шкала, тот же принцип разделения ролей.)*
