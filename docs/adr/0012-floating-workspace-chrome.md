...
Сейчас **нижняя часть главного окна** задана **жёсткой сеткой** `MainWindow`: строка с **полосой Workspace Health** (состояние воркспейса: сборка, тесты, отладка, git), сплиттер и **`BottomPanelView`** (терминал, сборка, отладка, git и т.д.) занимают **один вертикальный слой** (`MainGrid` row 5, внутренние строки `Auto` / сплиттер / `*`). Полоса **Workspace Health** (**`WorkspaceHealthStripView`**) физически **над** вкладками низа и **не плавает** — она привязана к той же колоночной разметке, что и рабочая область.

Следствие: любой **ситуационный** или **короткий** UI (статус, полоса Workspace Health, будущая компактная полоска отладки из [0011](0011-debug-situational-awareness.md)) оказывается в **одном бюджете высоты** с **длинным выводом** (лог, терминал). Поднять низ, чтобы что-то увидеть, — значит **отдать пиксели редактору**; это уже зафиксировано как продуктовая боль.
...
2. **Первые кандидаты на вынос из жёсткой нижней колонки (приоритет):**
   - **Workspace Health** (`WorkspaceHealthStripView` и логически родственное «узкое» состояние), сейчас застрявшие **над** `BottomPanelView` в одной нижней ячейке.
...
<a id="adr0012-p4"></a>
4. **Что остаётся привязанным к доку в v1 направления:** **длинный поток вывода** (терминал, журнал сборки, развёрнутый список locals/stack) — разумный дом для **нижней панели с вкладками**; цель ADR — не уничтожить её, а **не заставлять** короткий статус и полосу Workspace Health **обязательно** делить с ней одну ось высоты.
...
<a id="adr0012-p6"></a>
6. **Связь с режимами UI:** пресеты режимов ([0010](0010-ui-modes-toml-configuration.md)) могут задавать **видимость и слот** плавающего хрома (например в Debug показывать полоску отладки по умолчанию, в Focus — свернуть полосу Workspace Health в иконку), не переписывая всю схему `MainGrid` под каждый режим.
...
- Как не размножить **дубли Workspace Health** (полоска в Power и узкий режим) — одна модель данных, разные представления.
