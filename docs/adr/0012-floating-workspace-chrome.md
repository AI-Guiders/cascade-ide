...
Сейчас **нижняя часть главного окна** задана **жёсткой сеткой** `MainWindow`: строка с **полосой IDE Health** (сводка сборки/тестов/отладки/git — канал IDE Health), сплиттер и **`BottomPanelView`** (терминал, сборка, отладка, git и т.д.) занимают **один вертикальный слой** (`MainGrid` row 5, внутренние строки `Auto` / сплиттер / `*`). Полоса **IDE Health** (**`IdeHealthStripView`**) физически **над** вкладками низа и **не плавает** — она привязана к той же колоночной разметке, что и рабочая область.
...
2. **Первые кандидаты на вынос из жёсткой нижней колонки (приоритет):**
   - **IDE Health** (`IdeHealthStripView` и логически родственное «узкое» состояние), сейчас застрявшие **над** `BottomPanelView` в одной нижней ячейке.
