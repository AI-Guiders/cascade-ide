...
Сейчас **нижняя часть главного окна** задана **жёсткой сеткой** `MainWindow`: строка с телеметрией workspace, сплиттер и **`BottomPanelView`** (терминал, сборка, отладка, git и т.д.) занимают **один вертикальный слой** (`MainGrid` row 5, внутренние строки `Auto` / сплиттер / `*`). Полоса телеметрии (**`WorkspaceHealthStripView`**) физически **над** вкладками низа и **не плавает** — она привязана к той же колоночной разметке, что и рабочая область.
...
2. **Первые кандидаты на вынос из жёсткой нижней колонки (приоритет):**
   - **Workspace-телеметрия** (`WorkspaceHealthStripView` и логически родственное «узкое» состояние), сейчас застрявшие **над** `BottomPanelView` в одной нижней ячейке.
