...
Целевая секция, например **`[agent.roslyn_mcp]`**, отдельно от `[semantic_map]` и от `[[navigation.presets]]` без явного маппинга. Точные ключи — после прототипа; здесь зафиксированы **оси**.
