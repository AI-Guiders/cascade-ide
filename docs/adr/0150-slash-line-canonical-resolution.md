...
| Элемент | Состояние |
|---------|-----------|
| `SlashLineResolver` + `SlashLineResolverTests` | **В коде** |
| Autocomplete / Enter (`ChatSlashAutocomplete`) | **В коде** |
| Execute args (`ChatSlashCommandRunner`) | **В коде** |
| `arg_tail` в `intent-catalog.toml` | **Частично** (большинство команд — legacy `requires_arg_tail`) |

**In progress** до миграции каталога и стабилизации матрицы Enter/send на target hardware.
