...
**Дата:** 2026-04-12  
**Обновлено:** 2026-04-12 — «тема → подтемы», **обзор размаха на одном экране** vs глубокий скролл; смесь подзадач в одной сессии (ADR + ответвления) как норма.  
**Связь:** [0016](0016-agent-client-protocol-external-agent.md) (ACP / внешний агент), [0017](0017-multi-window-workspace-and-agent-surfaces.md) (зона MFD, `SecondaryShellView`, второе окно), [0021](0021-pfd-mfd-cockpit-attention-model.md) (модель внимания; чат во вторичном контуре), [0020](0020-agent-reasoning-visibility-and-provider-limits.md) (слои ответа и трасс), [0008](0008-mcp-contracts-and-testable-infrastructure.md) (контракты и паритет при расширении).
...
3. **Треды / темы** — отдельная ось: иметь смысл, когда пользователь ведёт **несколько параллельных линий** рассуждения или долгоживущие ветки. Для **одного** пакета вопросов в рамках одного шага плана чаще достаточно **одного блока с N полями** без полноценной модели тредов; треды не обязательны в v1 структурированных уточнений.

4. **Транспорт (ACP и др.):** расширение контракта сообщений/инструментов под структурированные пакеты — **в scope** вместе с поставкой UI, по аналогии с правилами [0008](0008-mcp-contracts-and-testable-infrastructure.md); до согласования — не подменять «настоящую» структуру одной строкой в метаданных.

5. **Связь с зонами:** основной чат остаётся во **вторичном контуре / MFD** ([0021](0021-pfd-mfd-cockpit-attention-model.md)); вынесение на второй монитор — [0017](0017-multi-window-workspace-and-agent-surfaces.md). Критичные подтверждения, требующие внимания PFD, — по политике [0017](0017-multi-window-workspace-and-agent-surfaces.md) п. 6, не через замаскированный «чат-да/нет».
...
- Переработка **Chat**-поверхности и, при необходимости, слоя состояния диалога в VM.
