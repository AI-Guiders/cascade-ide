...
| [naming-layers-v1.md](../design/naming-layers-v1.md) | `trace.*` semantic ids (§4) |
...
1. **Слоистая модель видимости** (концептуально; детали реализации — при интеграции конкретного провайдера). Канон **`trace_layer`** — [naming-layers-v1.md](../design/naming-layers-v1.md) §4; legacy L0–L2 — deprecated в UI:
   - **`trace.answer`** (legacy L0) — всегда: финальный ответ ассистента пользователю, факты об ошибках/отмене, **полные трассируемые действия** (вызовы MCP, git, терминал и т.д. — то, что IDE уже контролирует).
   - **`trace.reasoning`** (legacy L1) — по умолчанию включено, **если API отдаёт:** потоковые куски текста рассуждения, отдельные поля `thinking` / аналоги, явные «шаги» из структурированного ответа.
   - **`trace.provider_raw`** (legacy L2) — опционально / расширенный режим: сырой лог запросов-ответов к API (с редактированием секретов), диагностика токенов, тайминги — для отладки интеграции и доверия «что ушло и что пришло».
...
- Нужна ли отдельная **политика хранения** логов `trace.provider_raw` (локально только, TTL, исключение секретов).
