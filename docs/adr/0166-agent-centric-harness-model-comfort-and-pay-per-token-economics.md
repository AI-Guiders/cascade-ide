...
### 1.3 Cursor как reference, не как цель

| Сильнее в Cursor сегодня | Сильнее в CIDE (целевое) |
|--------------------------|---------------------------|
| Hooks: turn count, preCompact | AEE verify ladder + Verify Epoch |
| Зрелый MCP ecosystem в одном чате | Native IDE state + loopback MCP |
| Низкий порог setup | Topics / Intercom tree, compactor preset |
| Flat context economics | Прозрачная token economics + structured memory |

Parity hooks — **P1**; AEE — **уже дифференциатор**, его нельзя ослаблять ради «ещё одного чата».
...
|--------|------------------------|
| **agent-notes** | KB read/write; канон вне prompt |
...
**Правило:** native CIDE tools **дублируют критичное** из external MCP там, где нужен in-proc state (вкладка, epoch, debug).
...
| **agent-notes KB** | ADR summaries, decisions, open items |
...
|---|-------------|---------------------|
| P0.1 | **Harness preset** + idempotent setup (`Setup-CideHarness.ps1` или wizard) | Cloud.ru smoke + `mcp.json` + mode «CASA/Neumann T1» + compactor on |
