...
**Состояние реализации:** частично — `Features/Editor/Application` (адаптер AvaloniaEdit, `EditorStabilizedInputThrottler` **один на главное окно** в `MainWindowViewModel` + стабилизированный HUD в активной `DockDocumentView`, guard по `CurrentFilePath`), `SemanticProjectionPipeline` / `EditorSemanticSnapshot` из `WorkspaceDiagnosticsCoordinator`, `EditorHudEngine`; баннер HUD (`EditorHudBannerText`) использует снимок после стабилизированного ввода, при `DiagnosticsChanged` — инвалидация и `FromDiagnosticStrips`. Дальше — углубление DAL/inline vs banner (0085), без смены норм ADR.
