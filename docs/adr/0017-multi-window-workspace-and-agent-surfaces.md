...
- Минимальный **v1** второго `TopLevel` (вынесенная **зона Mfd** на другой монитор): семантика — всё ещё **вторичный контур** (`SecondaryShellView` в `Views/MainWindow.axaml`, `SecondaryShellPage` в `Models/SecondaryShellPage.cs`); **чат** — это **`SecondaryShellPage.Chat`**, одна из **страниц** хоста, не отдельная зона в модели [0021](0021-pfd-mfd-cockpit-attention-model.md). **Граница v1** — не «чат vs зона», а **масштаб хоста во втором окне:** **узкий** — один `TopLevel` с тем же типом хоста, но **зафиксирована одна страница** (например только Chat, без переключения на Terminal/Build/… в этом окне); **широкий** — полный **`SecondaryShellView`** с переключением **страниц**, как в колонке Mfd главного окна. Второе окно процесса — **`MfdHostWindow`** (роль `mfd_host` в MCP); **всё ещё открыто:** насколько полно хост совпадает с колонкой Mfd главного окна, сценарии открытия/плейсмента. В типичном сценарии вывод сборки — страница **Build** в том же хосте; инструментирование/полосы хрома (EICAS, Workspace Health и т.д.) задаются пресетом и могут потребовать отдельного решения при вынесении. **UX чата и терминала** — **отдельная итерация** (плотность, клавиатура); может идти параллельно и **не обязана** совпадать по сроку с первым вынесением. Выбор узкий/широкий хост — при реализации, **без** зависимости от переработки режимов (см. [«Уточнение: режимы UI»](#adr0017-modes-scope)).
