...
|--------|-------------------------|
| Разбор строки `presentation` | `Services/Presentation/PresentationParser.cs`, `PresentationGrammarTokens`, `PresentationLayoutAnalyzer`, `PresentationAnchorKind`; тесты `CascadeIDE.Tests/PresentationParserTests.cs`. |
...
| Жизненный цикл | Меню и команда `ToggleMfdHostWindow`; автозапуск при `Loaded` главного окна, если `PresentationRequestsMfdHostWindow` && `OpenMfdHostWindowOnStartup` && достаточно мониторов; при открытом хосте колонка MFD в `MainWindow` скрывается (`MainWindowViewModel.SetMfdHostWindowShellOpen`). |
