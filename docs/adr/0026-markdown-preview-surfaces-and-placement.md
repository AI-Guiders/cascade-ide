...
- **Предпочтительно в авторинге:** обычные Markdown-ссылки — `[см. п. 6](#adrNNNN-p6)` к якорю `<a id="adrNNNN-p6"></a>` в том же ADR; канон имён — [README ADR](README.md#adr-anchors-policy).
