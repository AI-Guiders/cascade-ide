...
| **Отрисовка вида** (лампа annunciator, полоса, …) | `AnnunciatorLampMetrics` + типы «лица» лампы (`*LampFace`), общий хром `AnnunciatorLampChrome` | Папка **`PrimitivesKit`** — собирательное имя слоя отрисовки; при рефакторинге допустимы имена вроде `…IndicatorDrawing`. |
| **Токены компоновки** / метрики сцены инструмента | например `CodeNavigationMapGraphPrimitives` | Это **не** вид индикатора в смысле deck — только числа раскладки/viewport. |
