...
- Семейство мелодий **Refactor → …** по диапазону (`rmx`, `rix`, …) и связка с Roslyn / **Roslyn MCP** (`roslyn_get_code_actions` / `roslyn_apply_code_action`) **не входят в обязательный минимум этого ADR** — см. [0110](0110-roslyn-refactor-intent-melody-bridge.md).
