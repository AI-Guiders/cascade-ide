...
| `rmx:5:20` | Refactor → Method → Extract по диапазону строк (см. [§3](#adr0081-p3)) |

Канон этой ADR-ветки:

- Для операций над диапазоном строк использовать модель **Editor → Line** (`el*`), без legacy/синонимов `es*`.
- Для рефакторингов использовать отдельную family **Refactor → Entity → Intent** (`r*`), например:
  - `rmx` = Refactor → Method → eXtract
  - `rix` = Refactor → Interface → eXtract
  - другие сущности/намерения — по мере появления в реестре alias.
