...
- Код wire-моделей (`CodeNavigationMapSubgraphDocument` и т.д.) **не обязан** переименовываться в `CodeNavigationSubgraphDocument`: префикс `SemanticMap*` остаётся про **артефакт композиции сцены**; уточняющий смысл «карта намерений кода» vs другие графы — через **`graph_kind`** (когда появится в контракте) и доменные поля.
