...
- применяет его к запросам к Roslyn MCP и к согласованному слою Semantic Map: **тактика** — PFD/Forward ([0053](0053-semantic-map-control-flow-pfd.md)); **стратегия (EFB)** — **MFD**, не PFD (см. [§3](#adr0059-p3));
...
**Норма зон:** **EFB — это режим и поверхность MFD**, не PFD. **PFD** ([0021](0021-pfd-mfd-cockpit-attention-model.md)) остаётся зоной **тактики** (текущий контекст, control flow, combat-declutter — §2). **MFD** — зона **стратегической** карты (глобальный/слойный скелет), в том числе на отдельном мониторе ([0017](0017-multi-window-workspace-and-agent-surfaces.md)).
...
|------|------|-----------------|-------------------|
| **Forward / PFD** | **Тактический** | Динамический профиль, узкий MCP, граф под курсор / `controlFlow` ([0053](0053-semantic-map-control-flow-pfd.md)). | Высокая (с учётом Echelon / debounce). |
...
**Инвариант:** экран EFB **не** «мигает» в ритме курсора и **не** разделяет с PFD одну и ту же цепочку автоматических эвристик — смысл **статики** и намеренного обновления. Раздельные частоты — **сопоставление поверхности** ([0017](0017-multi-window-workspace-and-agent-surfaces.md)) и **профиля**, не дублирование одного состояния.
