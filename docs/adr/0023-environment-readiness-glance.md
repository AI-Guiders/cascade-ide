...
1. Вводится отдельный продуктовый канал **готовность окружения** (рабочее имя в UI: по согласованию; в доках — *environment readiness / glance*). Он **не** использует префиксы `IdeHealth*` и не описывается ключами `workspace_health_*`.
