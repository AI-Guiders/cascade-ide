...
4. **Прецедент в репо:** `agent-notes-core` + общий исходник заметок — один слой доменной логики на **IDE** и **agent-notes-mcp** ([`AgentNotes.Core.csproj`](../../../agent-notes-core/AgentNotes.Core.csproj)).

## Решение (направление)

1. **Выделить библиотеку Git Core** (рабочее имя, например `GitMcp.Core` / `Cascade.Git.Core` — зафиксировать при внедрении) в том же монорепо/субмодульном контуре, что и git-mcp, по аналогии с `agent-notes-core`:
