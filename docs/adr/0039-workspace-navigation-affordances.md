...
**Agent/MCP (минимальная поверхность):** IDE-команда **`get_workspace_navigation_context`** — режимы **`related`** (список связанных файлов с кратким обоснованием) и **`subgraph`** (узлы и рёбра с численными капами). Якорь — активный документ в редакторе или явный `file_path`. Контракт и примеры в **`docs/MCP-PROTOCOL.md`**. Это **не** замена полноценной Semantic Map в UI, а **Pareto**-слой для агента поверх эвристик Roslyn и дерева решения.
