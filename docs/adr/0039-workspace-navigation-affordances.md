...
**Именованные пресеты фильтра** хранятся в **`%LocalAppData%\CascadeIDE\settings.toml`**, секция **`[workspace_navigation_context]`**, поле **`presets`** — JSON-объект: ключ — имя пресета, значение — опциональные массивы **`include_kinds`** / **`exclude_kinds`** (строковые идентификаторы видов связей). В коде заданы встроенные пресеты по умолчанию (`peers_only`, `no_namespace_noise`, `tests_and_peers`, `structure_only`); пользователь может переопределить или дополнить JSON в TOML.

**Аргументы MCP:** помимо режима и путей, поддерживаются **`preset`** (имя из `presets`), **`include_kinds`**, **`exclude_kinds`**. Правило merge: непустой **`include_kinds`** в запросе **заменяет** include пресета; **`exclude_kinds`** из запроса **объединяется** с exclude пресета (дедупликация по каноническому имени вида). Неизвестный пресет или непарсируемый JSON пресетов → ответ с **`error`: `bad_preset`**.
...
5. **MCP `get_workspace_navigation_context`: стабильные имена видов связей, пресеты пользователя, предсказуемый ответ.** Закрыто для **контракта агента**: канон строковых видов (`partial_peer`, …), хранение пресетов в `[workspace_navigation_context].presets` ([0028](0028-user-settings-toml-localappdata-and-secrets.md)), merge с аргументами вызова, эхо **`kind_filter`**, в **`subgraph`** — **`kind`** на узлах и **`related_kind`** на рёбрах. См. [§ Agent/MCP](#adr0039-mcp-workspace-navigation). Углубление эвристик и новые виды связей — итерации продукта, не ломая канон без мажорной версии контракта.
