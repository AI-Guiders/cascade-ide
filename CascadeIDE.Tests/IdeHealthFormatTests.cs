...
public sealed class WorkspaceHealthFormatTests
{
    ...
    {
        var idle = IdeHealthFormat.BuildSegment(isBuilding: false);
        Assert.Equal("Build: idle", idle.LineText);
        Assert.Equal("READY", idle.CockpitShort);
        Assert.False(idle.IsBuildRunning);

        var run = IdeHealthFormat.BuildSegment(isBuilding: true);
    ...
    {
        var withSummary = IdeHealthFormat.TestsSegment("ok 3 passed", 0);
        Assert.Equal("Tests: ok 3 passed", withSummary.LineText);
        Assert.Equal("ok 3 passed", withSummary.CockpitShort);

        var impacted = IdeHealthFormat.TestsSegment(null, 5);
    ...
    public void TestsSegment_truncates_long_summary_in_cockpit()
    {
        ...
        var s = IdeHealthFormat.TestsSegment(longText, 0);
    ...
    {
        var idle = IdeHealthFormat.DebugSegment(false, false, 0, 0);
        Assert.Equal("Debug: idle", idle.LineText);
        Assert.Equal("DBG · —", idle.CockpitShort);

        var paused = IdeHealthFormat.DebugSegment(true, true, 2, 4);
        ...
        var running = IdeHealthFormat.DebugSegment(true, false, 0, 0);
    ...
    {
        var snap = IdeHealthFormat.Compose(
            isBuilding: false,
            lastTestSummary: "",
