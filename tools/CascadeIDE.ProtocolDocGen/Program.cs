...
internal static class IdeMcpCommandExecutorEmitter
{
    ...
    public static string Emit()
    {
        ...
        sb.AppendLine();
        sb.AppendLine("namespace CascadeIDE.Features.IdeMcp.Execution;");
        sb.AppendLine();
        ...
        sb.AppendLine("        add(Services.IdeCommands.GetSolutionInfo, async (_, _) => await Task.FromResult(_actions.GetSolutionInfo()));");
        sb.AppendLine("        add(Services.IdeCommands.GetIdeState, async (_, _) => await _actions.GetIdeStateAsync());");
        sb.AppendLine("        add(Services.IdeCommands.GetCockpitSurface, async (_, _) => await _actions.GetCockpitSurfaceAsync());");
        sb.AppendLine("        add(Services.IdeCommands.GetUiModesDiagnostics, async (_, _) => await _actions.GetUiModesDiagnosticsAsync());");
        sb.AppendLine("        add(Services.IdeCommands.GetSolutionFiles, async (_, _) => await _actions.GetSolutionFilesAsync());");
        ...
        sb.AppendLine("        add(Services.IdeCommands.SearchWebPublicQuery, async (args, ct) => await _actions.SearchWebPublicQueryAsync(McpCommandJsonArgs.String(args, \"query\") ?? \"\", ct));");
        sb.AppendLine("        add(Services.IdeCommands.FetchWebPublicUrl, async (args, ct) => await _actions.FetchWebPublicUrlAsync(McpCommandJsonArgs.String(args, \"url\") ?? \"\", McpCommandJsonArgs.OptionalInt32(args, \"max_chars\"), ct));");
        ...
        sb.AppendLine("        add(Services.IdeCommands.GetCurrentFileDiagnostics, async (_, _) => await _actions.GetCurrentFileDiagnosticsAsync());");

        // Build / test
        sb.AppendLine("        add(Services.IdeCommands.Build, async (_, _) => await _actions.BuildAsync());");
        sb.AppendLine("        add(Services.IdeCommands.BuildStructured, async (_, _) => await _actions.BuildStructuredAsync());");
        sb.AppendLine("        add(Services.IdeCommands.RunTests, async (_, _) => await _actions.RunTestsAsync());");
        sb.AppendLine("        add(Services.IdeCommands.RunAffectedTests, async (args, _) => await _actions.RunAffectedTestsAsync(McpCommandJsonArgs.StringList(args, \"changed_paths\")));");
        sb.AppendLine("        add(Services.IdeCommands.RunCodeCleanup, async (args, _) => await _actions.RunCodeCleanupAsync(McpCommandJsonArgs.String(args, \"include_path\")));");
        sb.AppendLine("        add(Services.IdeCommands.GetCodeMetrics, async (args, _) => await _actions.GetCodeMetricsAsync(McpCommandJsonArgs.String(args, \"scope\"), McpCommandJsonArgs.String(args, \"path\")));");

        // Hybrid Codebase Index (паритет MCP hybrid-codebase-index)
        sb.AppendLine("        add(Services.IdeCommands.CodebaseIndexStatus, async (args, ct) => await _actions.CodebaseIndexStatusAsync(McpCommandJsonArgs.String(args, \"workspace_path\"), McpCommandJsonArgs.String(args, \"solution_path\"), ct));");
        ...
        sb.AppendLine("            return await _actions.CodebaseIndexSearchAsync(");
        ...
        sb.AppendLine("            return await _actions.CodebaseIndexExplainAsync(");
        sb.AppendLine("                McpCommandJsonArgs.String(args, \"workspace_path\"),");
        sb.AppendLine("                McpCommandJsonArgs.String(args, \"solution_path\"),");
        sb.AppendLine("                hitId.Value,");
        sb.AppendLine("                ct);");
        sb.AppendLine("        });");
        sb.AppendLine("        add(Services.IdeCommands.CodebaseIndexReindex, async (args, ct) => await _actions.CodebaseIndexReindexAsync(");
        sb.AppendLine("            McpCommandJsonArgs.String(args, \"workspace_path\"),");
        sb.AppendLine("            McpCommandJsonArgs.String(args, \"solution_path\"),");
        ...
        sb.AppendLine("        add(Services.IdeCommands.GitStatus, async (_, _) => await _actions.GitStatusAsync());");
        sb.AppendLine("        add(Services.IdeCommands.GitDiff, async (args, _) => await _actions.GitDiffAsync(McpCommandJsonArgs.String(args, \"path\"), McpCommandJsonArgs.Bool(args, \"staged\")));");
        sb.AppendLine("        add(Services.IdeCommands.GitLog, async (args, _) => await _actions.GitLogAsync(McpCommandJsonArgs.Int(args, \"n\", 20)));");
        sb.AppendLine("        add(Services.IdeCommands.GitFetch, async (args, _) => await _actions.GitFetchAsync(McpCommandJsonArgs.String(args, \"remote\"), McpCommandJsonArgs.Bool(args, \"all\"), McpCommandJsonArgs.Bool(args, \"prune\")));");
        sb.AppendLine("        add(Services.IdeCommands.GitPull, async (args, _) => await _actions.GitPullAsync(McpCommandJsonArgs.String(args, \"remote\"), McpCommandJsonArgs.String(args, \"branch\"), McpCommandJsonArgs.Bool(args, \"ff_only\", true)));");
        sb.AppendLine("        add(Services.IdeCommands.GitBranch, async (args, _) => await _actions.GitBranchAsync(McpCommandJsonArgs.String(args, \"action\"), McpCommandJsonArgs.String(args, \"name\"), McpCommandJsonArgs.String(args, \"start_point\"), McpCommandJsonArgs.Bool(args, \"force\")));");
        ...
        sb.AppendLine("            return await _actions.GitShowAsync(rev, McpCommandJsonArgs.String(args, \"path\"), McpCommandJsonArgs.Bool(args, \"stat_only\"));");
        sb.AppendLine("        });");
        sb.AppendLine("        add(Services.IdeCommands.GitSubmodule, async (args, _) => await _actions.GitSubmoduleAsync(McpCommandJsonArgs.String(args, \"action\"), McpCommandJsonArgs.String(args, \"path\"), McpCommandJsonArgs.Bool(args, \"recursive\", true)));");
        ...
        sb.AppendLine("            return await _actions.GitCommitAsync(McpCommandJsonArgs.String(args, \"message\")!, McpCommandJsonArgs.StringList(args, \"paths\"));");
        sb.AppendLine("        });");
        sb.AppendLine("        add(Services.IdeCommands.GitPush, async (args, _) => await _actions.GitPushAsync(McpCommandJsonArgs.String(args, \"remote\"), McpCommandJsonArgs.String(args, \"branch\")));");

        // Output / diagnostics
        sb.AppendLine("        add(Services.IdeCommands.GetBuildOutput, async (_, _) => await Task.FromResult(_actions.GetBuildOutput()));");

        // UI inspection / control (pure IIdeMcpActions)
        sb.AppendLine("        add(Services.IdeCommands.GetUiTheme, async (_, _) => await Task.FromResult(_actions.GetUiTheme()));");
        sb.AppendLine("        add(Services.IdeCommands.SetUiTheme, async (args, _) => await _actions.SetUiThemeAsync(McpCommandJsonArgs.String(args, \"theme\") ?? \"\"));");
        sb.AppendLine("        add(Services.IdeCommands.GetUiLayout, async (_, _) => await _actions.GetUiLayoutAsync());");
        sb.AppendLine("        add(Services.IdeCommands.GetColorsUnderCursor, async (_, _) => await _actions.GetColorsUnderCursorAsync());");
        sb.AppendLine("        add(Services.IdeCommands.GetControlAppearance, async (args, _) => await _actions.GetControlAppearanceAsync(McpCommandJsonArgs.String(args, \"name\")));");
        ...
        sb.AppendLine("            return await _actions.SetControlLayoutAsync(McpCommandJsonArgs.String(args, \"name\")!, McpCommandJsonArgs.String(args, \"layout\") ?? \"{}\" );");
        sb.AppendLine("        });");
        sb.AppendLine("        add(Services.IdeCommands.SetControlText, async (args, _) => await _actions.SetControlTextAsync(McpCommandJsonArgs.String(args, \"name\") ?? \"\", McpCommandJsonArgs.String(args, \"text\") ?? \"\"));");
        sb.AppendLine("        add(Services.IdeCommands.ClickControl, async (args, _) => await _actions.ClickControlAsync(McpCommandJsonArgs.String(args, \"name\")));");
        sb.AppendLine("        add(Services.IdeCommands.SendKeys, async (args, _) => await _actions.SendKeysAsync(McpCommandJsonArgs.String(args, \"name\"), McpCommandJsonArgs.String(args, \"keys\") ?? \"\"));");
        sb.AppendLine("        add(Services.IdeCommands.SetFocus, async (args, _) => await _actions.SetFocusAsync(McpCommandJsonArgs.String(args, \"name\")));");
        sb.AppendLine("        add(Services.IdeCommands.HighlightControl, async (args, _) => await _actions.HighlightControlAsync(McpCommandJsonArgs.String(args, \"name\")));");
        ...
        sb.AppendLine("            return await _actions.SetPanelSizeAsync(McpCommandJsonArgs.String(args, \"panel\") ?? \"\", w, h);");
        sb.AppendLine("        });");
        sb.AppendLine("        add(Services.IdeCommands.GetSupportedEditorLanguages, async (_, _) => await Task.FromResult(_actions.GetSupportedEditorLanguages()));");
        sb.AppendLine("        add(Services.IdeCommands.ChatExportReadable, async (args, _) => await _actions.ExportChatReadableAsync(McpCommandJsonArgs.Bool(args, \"write_file\", false), McpCommandJsonArgs.String(args, \"file_name\")));");
