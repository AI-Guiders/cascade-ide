# Live dogfood: Radio → /intercom message find + anchors → PrintWindow
...
foreach ($n in @('Radio', '#crew', 'Crew', 'Intercom')) {
    try { Click-Name $n; Write-Output "clicked=$n"; break } catch { }
}
...
$composer = $null
foreach ($e in $edits) { if ($e.Current.IsEnabled) { $composer = $e; break } }
...
$val.SetValue('/intercom message find CascadeIDE.GlassCore/Intercom/GlassIntercomMessageFind.cs:10')
Start-Sleep -Milliseconds 400
Click-Name 'Send'
Start-Sleep -Seconds 1

$val.SetValue('/intercom message anchors')
Start-Sleep -Milliseconds 400
Click-Name 'Send'
Start-Sleep -Seconds 1

$out = Join-Path $PSScriptRoot '..\tmp-glass-shots\a4-message-find-anchors-20260806.png'
...
Write-Output "PNG=$out exists=$(Test-Path $out) pid=$($p.Id)"
