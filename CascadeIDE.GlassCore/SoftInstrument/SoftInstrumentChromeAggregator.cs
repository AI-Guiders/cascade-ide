...
/// <summary>
/// SoftInstrument chrome_hint latch store for Glass top band.
/// Density math lives in <see cref="SoftInstrumentChromeDensityPolicy"/> (shared with Avalonia).
/// </summary>
public sealed class SoftInstrumentChromeAggregator
{
    public const int MaxVisible = SoftInstrumentChromeDensityPolicy.DefaultMaxVisible;
    ...
    public bool ToggleExpanded()
    {
        lock (_gate)
        {
            _expanded = SoftInstrumentChromeDensityPolicy.ToggleExpanded(_expanded, HintCountUnlocked());
            return _expanded;
        }
    }
    ...
    public Band Snapshot()
    {
        lock (_gate)
        {
            var r = SoftInstrumentChromeDensityPolicy.Collapse(HintCandidatesUnlocked(), expanded: _expanded);
    ...
    public ChipBand SnapshotChips(int maxVisible = 6)
    {
        lock (_gate)
        {
            ...
            static Chip[] Map(IEnumerable<SoftInstrumentChromeDensityPolicy.Hint> hints) =>
                hints.Select(h => new Chip(
                    h.Id,
                    SoftInstrumentChromeDensityPolicy.ShortLabel(h.Id),
                    h.Text,
                    SoftInstrumentChromeDensityPolicy.ChipLevelFromHint(h.Text))).ToArray();
    ...
    /// <summary>Store or clear chrome_hint for a catalog SoftInstrument id; unknown ids are ignored.</summary>
    public void Apply(string organId, string? chromeHint)
    {
        var id = SoftInstrumentLatchCatalog.Canonicalize(organId);
        if (!SoftInstrumentLatchCatalog.Contains(id))
            return;
    ...
    IEnumerable<SoftInstrumentChromeDensityPolicy.Hint> HintCandidatesUnlocked()
    {
        foreach (var kv in _hints)
        {
            if (SoftInstrumentChromeDensityPolicy.From(kv.Key, kv.Value) is { } h)
                yield return h;
        }
    }

    /// <summary>Lower number = higher priority (shared SoftInstrument table).</summary>
