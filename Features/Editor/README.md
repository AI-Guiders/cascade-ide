...
- **`Application/Presentation/`** — `EditorHudBannerTextComposer` (file-level баннер 0085); `EditorInlineHoverToolTipController` (inline hover); **`EditorInlineHoverChrome`** (тайминги/смещения `ToolTip` на `TextEditor`, не IDS/0079); `EditorDocumentBackgroundVisualsHandle` (регистрация `IBackgroundRenderer`: squiggles + debug/breakpoint из `Services/`); **`EditorInlineHudLayer.InstallDocumentBackgroundVisuals`** — фасад (см. `docs/design/editor-hud-inline-migration-inventory-v1.md`).
