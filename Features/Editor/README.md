...
- **`Application/Presentation/`** — `EditorHudBannerTextComposer` (file-level баннер 0085); `EditorInlineHoverToolTipController` (inline hover); `EditorDocumentBackgroundVisualsHandle` (регистрация `IBackgroundRenderer`: squiggles + debug/breakpoint из `Services/`); **`EditorInlineHudLayer.InstallDocumentBackgroundVisuals`** — фасад (см. `docs/design/editor-hud-inline-migration-inventory-v1.md`).
