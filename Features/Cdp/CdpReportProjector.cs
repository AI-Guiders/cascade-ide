...
/// <summary>
/// Agent report board pulse → quiet chrome band.
/// Watches report-LATEST.json; paints <see cref="MainWindowViewModel.AgentReportChromeHint"/>.
/// Idle (no chrome_hint) stays silent — not EICAS.
/// </summary>
internal sealed class CdpReportProjector : IDisposable
{
    ...
    CdpReportProjector(MainWindowViewModel vm, string stateRoot)
    {
        _vm = vm;
        Directory.CreateDirectory(stateRoot);
        _watcher = new FileSystemWatcher(stateRoot)
        {
            Filter = "report-LATEST.json",
    ...
    public static CdpReportProjector Start(MainWindowViewModel vm)
    {
        Instance?.Dispose();
        Instance = new CdpReportProjector(vm, StateRoot);
        return Instance;
    }
    ...
    void TryApplyFromDisk(bool force)
    {
        ...
        try
        {
            _vm.ApplyReportChromeHint(hint);
        }
        catch
        {
            /* best-effort */
