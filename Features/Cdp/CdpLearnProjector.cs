...
/// <summary>
/// Agent learn journal pulse → quiet chrome band.
/// Watches learn-LATEST.json; paints <see cref="MainWindowViewModel.AgentLearnChromeHint"/>.
/// Idle (no chrome_hint) stays silent — not EICAS.
/// </summary>
internal sealed class CdpLearnProjector : IDisposable
{
    ...
    public static CdpLearnProjector? Instance { get; private set; }
    ...
    CdpLearnProjector(MainWindowViewModel vm, string stateRoot)
    {
        _vm = vm;
        Directory.CreateDirectory(stateRoot);
        _watcher = new FileSystemWatcher(stateRoot)
        {
            Filter = "learn-LATEST.json",
    ...
    public static CdpLearnProjector Start(MainWindowViewModel vm)
    {
        Instance?.Dispose();
        Instance = new CdpLearnProjector(vm, StateRoot);
        return Instance;
    }
