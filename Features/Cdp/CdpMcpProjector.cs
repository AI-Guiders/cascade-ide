...
/// <summary>
/// Agent MCP outlet pulse → quiet chrome band.
/// Watches mcp-LATEST.json; paints <see cref="MainWindowViewModel.AgentMcpChromeHint"/>.
/// Idle (no chrome_hint) stays silent — not EICAS.
/// </summary>
internal sealed class CdpMcpProjector : IDisposable
{
    ...
    public static CdpMcpProjector? Instance { get; private set; }
    ...
    CdpMcpProjector(MainWindowViewModel vm, string stateRoot)
    {
        _vm = vm;
        Directory.CreateDirectory(stateRoot);
        _watcher = new FileSystemWatcher(stateRoot)
        {
            Filter = "mcp-LATEST.json",
    ...
    public static CdpMcpProjector Start(MainWindowViewModel vm)
    {
        Instance?.Dispose();
        Instance = new CdpMcpProjector(vm, StateRoot);
        return Instance;
    }
    ...
    void TryApplyFromDisk(bool force)
    {
        ...
        try
        {
            ...
            {
                _vm.ApplyMcpChromeHint(null);
                return;
            }

            var raw = File.ReadAllText(LatchPath);
            doc = JsonSerializer.Deserialize<McpLatchDoc>(raw, ReadOpts);
        }
        catch
        {
            return;
