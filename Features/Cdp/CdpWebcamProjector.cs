...
/// <summary>
/// Agent webcam capture pulse → quiet chrome band.
/// Watches webcam-LATEST.json; paints <see cref="MainWindowViewModel.AgentWebcamChromeHint"/>.
/// Idle (no chrome_hint) stays silent — not EICAS.
/// </summary>
internal sealed class CdpWebcamProjector : IDisposable
{
    ...
    public static CdpWebcamProjector? Instance { get; private set; }
    ...
    CdpWebcamProjector(MainWindowViewModel vm, string stateRoot)
    {
        _vm = vm;
        Directory.CreateDirectory(stateRoot);
        _watcher = new FileSystemWatcher(stateRoot)
        {
            Filter = "webcam-LATEST.json",
    ...
    public static CdpWebcamProjector Start(MainWindowViewModel vm)
    {
        Instance?.Dispose();
        Instance = new CdpWebcamProjector(vm, StateRoot);
        return Instance;
    }
    ...
    void TryApplyFromDisk(bool force)
    {
        ...
        try
        {
            _vm.ApplyWebcamChromeHint(hint);
        }
        catch
        {
            /* best-effort */
