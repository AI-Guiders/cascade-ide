...
/// <summary>
/// Agent Debug-SA pulse → quiet chrome band.
/// Watches debug_desk-LATEST.json; paints <see cref="MainWindowViewModel.AgentDebugDeskChromeHint"/>.
/// Idle (no chrome_hint) stays silent — not EICAS. Live DebugStack host = <c>DebugStackMfdPageView</c>.
/// </summary>
internal sealed class CdpDebugDeskProjector : IDisposable
{
    ...
    public static CdpDebugDeskProjector? Instance { get; private set; }
    ...
    CdpDebugDeskProjector(MainWindowViewModel vm, string stateRoot)
    {
        _vm = vm;
        Directory.CreateDirectory(stateRoot);
        _watcher = new FileSystemWatcher(stateRoot)
        {
            Filter = "debug_desk-LATEST.json",
    ...
    public static CdpDebugDeskProjector Start(MainWindowViewModel vm)
    {
        Instance?.Dispose();
        Instance = new CdpDebugDeskProjector(vm, StateRoot);
        return Instance;
    }
    ...
    void TryApplyFromDisk(bool force)
    {
        ...
        DebugDeskLatchDoc? doc;
        try
        {
            var raw = CdpLatchIo.TryReadAllTextIfExists(LatchPath);
            if (raw is null)
            {
                _vm.ApplyDebugDeskChromeHint(null);
                return;
            }
            doc = JsonSerializer.Deserialize<DebugDeskLatchDoc>(raw, ReadOpts);
        }
        catch
        {
            return;
        ...
        try
        {
            _vm.ApplyDebugDeskChromeHint(hint);
        }
        catch
        {
            /* best-effort */
