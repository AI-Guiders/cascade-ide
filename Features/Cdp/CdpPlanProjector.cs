...
/// <summary>
/// Agent Task Manager (plan) pulse → quiet chrome band.
/// Watches plan-LATEST.json; paints <see cref="MainWindowViewModel.AgentPlanChromeHint"/>.
/// Idle (no chrome_hint) stays silent — not EICAS.
/// </summary>
internal sealed class CdpPlanProjector : IDisposable
{
    ...
    public static CdpPlanProjector? Instance { get; private set; }
    ...
    CdpPlanProjector(MainWindowViewModel vm, string stateRoot)
    {
        _vm = vm;
        Directory.CreateDirectory(stateRoot);
        _watcher = new FileSystemWatcher(stateRoot)
        {
            Filter = "plan-LATEST.json",
    ...
    public static CdpPlanProjector Start(MainWindowViewModel vm)
    {
        Instance?.Dispose();
        Instance = new CdpPlanProjector(vm, StateRoot);
        return Instance;
    }
    ...
    void TryApplyFromDisk(bool force)
    {
        ...
        try
        {
            ...
            if (raw is null)
            {
                _vm.ApplyPlanChromeHint(null);
                return;
            }
            doc = JsonSerializer.Deserialize<PlanLatchDoc>(raw, ReadOpts);
        }
        catch
        {
            return;
