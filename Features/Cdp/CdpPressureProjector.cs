...
/// <summary>
/// Agent L1 pressure continuity → quiet chrome band.
/// Watches pressure-LATEST.json; paints <see cref="MainWindowViewModel.AgentPressureChromeHint"/>.
/// Idle (no chrome_hint) stays silent — not EICAS.
/// </summary>
internal sealed class CdpPressureProjector : IDisposable
{
    ...
    public static CdpPressureProjector? Instance { get; private set; }
    ...
    CdpPressureProjector(MainWindowViewModel vm, string stateRoot)
    {
        _vm = vm;
        Directory.CreateDirectory(stateRoot);
        _watcher = new FileSystemWatcher(stateRoot)
        {
            Filter = "pressure-LATEST.json",
    ...
    public static CdpPressureProjector Start(MainWindowViewModel vm)
    {
        Instance?.Dispose();
        Instance = new CdpPressureProjector(vm, StateRoot);
        return Instance;
    }
    ...
    void TryApplyFromDisk(bool force)
    {
        ...
        PressureLatchDoc? doc;
        try
        {
            if (!File.Exists(LatchPath))
            {
                _vm.ApplyPressureChromeHint(null);
                return;
            }

            var raw = File.ReadAllText(LatchPath);
            doc = JsonSerializer.Deserialize<PressureLatchDoc>(raw, ReadOpts);
        }
        catch
        {
            return;
        ...
        try
        {
            _vm.ApplyPressureChromeHint(hint);
        }
        catch
        {
            /* best-effort */
