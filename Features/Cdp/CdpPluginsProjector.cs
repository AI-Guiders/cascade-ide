...
/// <summary>
/// Agent plugins attention pulse → quiet chrome band.
/// Watches plugins-LATEST.json; paints <see cref="MainWindowViewModel.AgentPluginsChromeHint"/>.
/// Idle (no chrome_hint) stays silent — not EICAS.
/// </summary>
internal sealed class CdpPluginsProjector : IDisposable
{
    ...
    public static CdpPluginsProjector? Instance { get; private set; }
    ...
    CdpPluginsProjector(MainWindowViewModel vm, string stateRoot)
    {
        _vm = vm;
        Directory.CreateDirectory(stateRoot);
        _watcher = new FileSystemWatcher(stateRoot)
        {
            Filter = "plugins-LATEST.json",
    ...
    public static CdpPluginsProjector Start(MainWindowViewModel vm)
    {
        Instance?.Dispose();
        Instance = new CdpPluginsProjector(vm, StateRoot);
        return Instance;
    }
    ...
    void TryApplyFromDisk(bool force)
    {
        ...
        try
        {
            _vm.ApplyPluginsChromeHint(hint);
        }
        catch
        {
            /* best-effort */
