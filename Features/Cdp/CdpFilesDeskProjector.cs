...
internal sealed class CdpFilesDeskProjector : IDisposable
{
    ...
    public static CdpFilesDeskProjector? Instance { get; private set; }
    ...
    CdpFilesDeskProjector(MainWindowViewModel vm, string stateRoot)
    {
        _vm = vm;
        Directory.CreateDirectory(stateRoot);
        _watcher = new FileSystemWatcher(stateRoot)
        {
            Filter = "files_desk-LATEST.json",
    ...
    public static CdpFilesDeskProjector Start(MainWindowViewModel vm)
    {
        Instance?.Dispose();
        Instance = new CdpFilesDeskProjector(vm, StateRoot);
        return Instance;
    }
    ...
    void TryApplyFromDisk(bool force)
    {
        ...
        FilesDeskLatchDoc? doc;
        try
        {
            var raw = CdpLatchIo.TryReadAllTextIfExists(LatchPath);
            if (raw is null)
            {
                _vm.ApplyFilesDeskChromeHint(null);
                return;
            }
            doc = JsonSerializer.Deserialize<FilesDeskLatchDoc>(raw, ReadOpts);
        }
        catch
        {
            return;
        ...
        try
        {
            _vm.ApplyFilesDeskChromeHint(hint);
        }
        catch
        {
            /* best-effort */
