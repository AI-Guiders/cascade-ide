...
/// <summary>
/// Agent domain ownership pulse → quiet chrome band.
/// Watches domain-LATEST.json; paints <see cref="MainWindowViewModel.AgentDomainChromeHint"/>.
/// Idle (no chrome_hint) stays silent — not EICAS.
/// </summary>
internal sealed class CdpDomainProjector : IDisposable
{
    ...
    public static CdpDomainProjector? Instance { get; private set; }
    ...
    CdpDomainProjector(MainWindowViewModel vm, string stateRoot)
    {
        _vm = vm;
        Directory.CreateDirectory(stateRoot);
        _watcher = new FileSystemWatcher(stateRoot)
        {
            Filter = "domain-LATEST.json",
    ...
    public static CdpDomainProjector Start(MainWindowViewModel vm)
    {
        Instance?.Dispose();
        Instance = new CdpDomainProjector(vm, StateRoot);
        return Instance;
    }
    ...
    void TryApplyFromDisk(bool force)
    {
        ...
        try
        {
            ...
            if (raw is null)
            {
                _vm.ApplyDomainChromeHint(null);
                return;
            }
            doc = JsonSerializer.Deserialize<DomainLatchDoc>(raw, ReadOpts);
        }
        catch
        {
            return;
        ...
        try
        {
            _vm.ApplyDomainChromeHint(hint);
        }
        catch
        {
            /* best-effort */
