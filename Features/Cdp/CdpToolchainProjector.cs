...
/// <summary>
/// Agent toolchain health pulse → quiet chrome band.
/// Watches toolchain-LATEST.json; paints <see cref="MainWindowViewModel.AgentToolchainChromeHint"/>.
/// Idle (no chrome_hint) stays silent — not EICAS.
/// </summary>
internal sealed class CdpToolchainProjector : IDisposable
{
    ...
    public static CdpToolchainProjector? Instance { get; private set; }
    ...
    CdpToolchainProjector(MainWindowViewModel vm, string stateRoot)
    {
        _vm = vm;
        Directory.CreateDirectory(stateRoot);
        _watcher = new FileSystemWatcher(stateRoot)
        {
            Filter = "toolchain-LATEST.json",
    ...
    public static CdpToolchainProjector Start(MainWindowViewModel vm)
    {
        Instance?.Dispose();
        Instance = new CdpToolchainProjector(vm, StateRoot);
        return Instance;
    }
    ...
    void TryApplyFromDisk(bool force)
    {
        ...
        ToolchainLatchDoc? doc;
        try
        {
            if (!File.Exists(LatchPath))
            {
                _vm.ApplyToolchainChromeHint(null);
                return;
            }

            var raw = File.ReadAllText(LatchPath);
            doc = JsonSerializer.Deserialize<ToolchainLatchDoc>(raw, ReadOpts);
        }
        catch
        {
            return;
        ...
        try
        {
            _vm.ApplyToolchainChromeHint(hint);
        }
        catch
        {
            /* best-effort */
