...
[IoBoundary]
internal sealed class WindowsConPtyIntegratedShellSession(ShellLaunchConfiguration launch) : IIntegratedShellSession
{
    ...
    public void Resize(int cols, int rows)
    {
        ...
        try
        {
            var result = NativeMethods.ResizePseudoConsole(
                pseudoConsoleHandle,
                new Coord((short)normalized.cols, (short)normalized.rows));
            if (result != 0)
                Marshal.ThrowExceptionForHR(result);
        }
    ...
    private void StartProcessAttachedToPseudoConsole(WindowsPseudoConsoleSafeHandle pseudoConsole)
    {
        ...
        try
        {
            ...
            commandLine = Marshal.StringToHGlobalUni(BuildCommandLine(launch));
            ...
            IntegratedShellLaunch.ApplyUtf8ConsoleEnvironment(environmentVariables);
            ...
            if (!NativeMethods.CreateProcess(
                    ...
                    lpCurrentDirectory: launch.WorkingDirectory,
    ...
    private static string BuildCommandLine(ShellLaunchConfiguration launch) =>
        string.Join(" ", new[] { QuoteArgument(launch.FileName) }.Concat(launch.Arguments.Select(QuoteArgument)));
