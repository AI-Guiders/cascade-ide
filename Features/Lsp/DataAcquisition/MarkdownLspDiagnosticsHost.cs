...
public sealed class MarkdownLspDiagnosticsHost : ILspDiagnosticSource
{
    ...
    /// <summary>
    /// Запуск Marksman (или custom exe), <c>initialize</c> с <paramref name="solutionPath"/> → корень workspace,
    /// затем <c>initialized</c>.
    /// </summary>
    public async Task<bool> TryStartAsync(string providerId, string? solutionPath, string? userExecutable, string? userArguments, CancellationToken ct)
    {
        ...
        if (string.Equals(providerId, MarkdownLspProviderIds.Off, StringComparison.OrdinalIgnoreCase))
            return false;

        if (string.IsNullOrWhiteSpace(solutionPath) || !File.Exists(solutionPath))
            return false;

        var (exe, args) = MarkdownLspProviderIds.ResolveProcessArgs(providerId, userExecutable, userArguments);
        if (string.IsNullOrWhiteSpace(exe))
            return false;

        var workspaceRoot = BreakpointsFileService.GetWorkspaceRoot(solutionPath);
        ...
        try
        {
            var rootUri = LspFileUri.PathToFileUri(workspaceRoot);
    ...
    {
        if (!IsActive || string.IsNullOrEmpty(filePath) || !IsMarkdownFile(filePath))
            return;
        var key = LspFileUri.NormalizePath(filePath);
        lock (_gate)
        {
    ...
    private static string FormatCode(JsonElement d)
    {
        ...
        {
            JsonValueKind.String => c.GetString() ?? "lsp-md",
