...
## Invariants
- Human-faced SoftInstrument zones = tree / graph / instrument cards — not status/ribbon text dumps.
- SoftInstrument seats (P|F|M) = chrome only; never steal MFD page.
...
- Document Face invite page = `GlassFacePagePolicy` (not MainWindow `EndsWith(.md)`); SoftInstrument Face stays glance cards — not MarkdownPreview wall.

## Entry
- Map SSOT: `scratch/softinstrument-human-viz-map-2026-08-04.md`
...
## last_ship
- **2026-08-08 Glass Done PreCondition STAMPED** — dig=`glass.md` PreCondition + plan ALL SURFACES + SoftInstrument Face Done · PrintWindow `CDP GlassCockpit · Windows` Soft:QRH cards live · evidence `tmp-glass-shots/glass-done-precondition-20260808.png` + `cdp_see`. SoftInstrument Meta CLOSED.
- **2026-08-08 SoftInstrument Face Done axis4** — Soft:QRH cards VERIFY (M·QRH situations+find) · dig=`tmp-glass-shots/softinstrument-qrh-face-cards-20260808.png` + `cdp_see` · operator in-thread (chat open ≠ Soft:QRH fail; nested axb steer) · SoftInstrument Meta CLOSED.
- **2026-08-08 SoftFL Soft:QRH Face cards VERIFY** — Soft:QRH → `M · QRH` · situations cards (cabin-start/open-project/where-am-i/hung-glass) · find · not markdown wall. Evidence `tmp-glass-shots/softinstrument-qrh-face-cards-20260808.png` (title=`CDP GlassCockpit · Windows`). SoftInstrument Meta CLOSED. Face Done axis4 = Soft:QRH VERIFY + operator steer.
- **2026-08-08 Glass Open/SE = CIDE SolutionParser SSOT** — linked `SolutionParser`+`ProjectFileTreeBuilder`+`FolderWorkspaceTreeBuilder`+`SolutionItem` into GlassCore; `GlassSession.SetSolutionOrProjectPath` = CIDE LoadSolution; WPF SE paints `SolutionItem` children. Evidence path: Ctrl+O→P→CdpMcp.csproj → tree. SoftInstrument Meta CLOSED.
- **2026-08-08 W0 Open/Load SoftFL** — Ctrl+O → F/P/D/R overlay · open file/sln/folder · MRU recent · `SetWorkspaceRoot` + SE refresh. Catalogs palette/chord `os/od/or/oo` · QRH «открыть проект» steps. Evidence `tmp-glass-shots/open-family-chord-20260808.png`. Plan `.cdp/plans/glass-cide-port-glance.md`. SoftInstrument Meta CLOSED · invent REJECT.
- **2026-08-08 SoftFL HERE/NEXT + Soft:QRH situations→steps (nested axb)** — operator: label deck ≠ «что дальше»; want aircraft QRH + always HERE. Ship: `OperatorSituationCatalog` (situations+steps) · SoftInstrument Face cards = situations (click→шаг N/M · дальше/назад/сделать/к списку) · MFD `HereNext` + palette `HERE/NEXT` + chord `hn` (GlassChordCatalog fallback) · HERE line locus. Build Release 0 err · Add-Type ChipsFor ok · evidence `tmp-glass-shots/softinstrument-qrh-situations-20260808.png` + `softinstrument-here-next-20260808.png` (CDP GlassCockpit · Windows · M·HereNext · шаг 1/4). dig=`ADR 0014`+`softinstrument-human-viz` · SoftInstrument Meta CLOSED.
- **2026-08-08 SoftFL SoftInstrument Face cards+find** — operator eyes: QRH/ECL markdown wall ≠ one glance (Plan is the bar). Ship: `OpenSoftInstrumentFace` → MFD `QRH|ECL|Alert` glance deck (`SoftInstrumentFaceHandbook.ChipsFor` + `SoftInstrumentFindBox` filter) · not MarkdownPreview. Build Release ok · Add-Type ChipsFor ok · live PNG owed (Capture-Window hung; operator glance). dig=`softinstrument-human-viz` · SoftInstrument Meta CLOSED.
- **2026-08-07 SoftFL Glass C-q QRH/ECL/alert Face path** — Ctrl+Q discover+run SoftInstrument EICAS family without SoftInstrument Meta reopen. `OpenSoftInstrumentFace` → citizen `@intent qrh|ecl|alert` PlaceOrgan · evidence `tmp-glass-shots/softfl-cq-palette-qrh-20260807.png` (palette Soft: QRH) · dig=`GlassCommandPaletteCatalog.cs`.
- **2026-08-07 Glass human-viz nested SoftFL CLOSED** — operator ask finish SoftInstrument human viz via nested[axb]. Lived residual: ProductSpineStrip `Glass PreCondition · A6 ADOPTED denser…` + MarkdownPreview pressure-LATEST dump. SoftFL: `ChatProductSpinePresentation.FormatFaceStrip` · MD drop pressure fallback · EICAS `FormatEicasFace` SoftFL jargon strip · latch spine humanized. Unit Add-Type: `Glass Done · message select ready`. Live PNG `tmp-glass-shots/softinstrument-nested-after-softfl-20260807.png` (spine=`Glass Done · SoftInstrument human viz · instruments people can fly`) + MFD sweep `tmp-glass-shots/softinstrument-sweep-{problems,flightdatastorage,domainboard,editor,markdownpreview}-20260807.png`. Map ok zones = verify not invent. SoftInstrument human-viz epic DONE.
...
- **2026-08-07 Face theatre strip across LatchPaint peels** — Plan WHY/NEXT/board · SoftInstrument chrome · Seats tip · Intercom compact · IgniteWake tip · slash SoftFL jargon out · paired cdp-mcp IdeHumanFacePlan Why/Next/Pulse/Board on PublishGlass · no operator-eyes theatre on Face
